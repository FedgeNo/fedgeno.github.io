...
**What remains open.** Whether the Dirac operator $D$ on $M_\infty$ decomposes cleanly as $D_t + D_\Xi$ or carries mixed terms is *not* an independent question. The Levi-Civita connection — and hence the spin connection and $D$ — mixes the $\mathbb{R}_t$ and $\Xi_\infty$ factors **iff** the metric is not an exact product, i.e. iff it has nonzero cross-blocks $g_{\mu a}$ or cross-coordinate dependence between the factors. So $D = D_t + D_\Xi$ with no mixed terms holds **iff** $M_\infty = \mathbb{R}_t \times \Xi_\infty$ is an *exact* product; the operator-form question and the product-metric question are one and the same (Appendix A §9). The exactness of the product structure is derived at vacuum level, weak-field rigor (`files/idwt.py` STEP 133): the vacuum is static, spatially uniform, time-reversal invariant, and isotropic on each sector fiber, so the cross-blocks of its stress tensor vanish by symmetry — the spatial rows by spatial isotropy, the time row by T-invariance, and equivalently any isometry-invariant fiber vector vanishes (the $\mathrm{U}(k)$ center kills it on $\mathbb{CP}^k$ fibers; $\mathrm{SO}(m)$ transitivity on $S^m$) — and the stationary metric inherits the vacuum symmetry by the Part 4 §3.8 uniqueness argument, giving $g_{\mu a} = 0$ exactly for the vacuum solution. Away from vacuum the cross-blocks are standard weak-field matter corrections, whose linear suppression is established (Part 4 §3.8). Residual (🔶): nonperturbative uniqueness, shared with §3.8. The exactness question no longer waits on the sector-selection program.
