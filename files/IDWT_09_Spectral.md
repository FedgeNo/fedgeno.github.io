...
**Note.** The spectral triple notation $(\mathcal{A},\mathcal{H},D)$ is used here as a convenient mathematical framework. IDWT is not a noncommutative geometry (NCG) model in the Connes-Marcolli sense. This section defines the IDWT spectral triple proposal and the structure it is expected to have. The operator-theoretic properties below are now established for the **physically relevant operator** — the finite-active internal triple, equivalently the projected $P_{\xi^0}DP_{\xi^0}$ — with status recorded as of the 2026-06-13 spectral-triple pass (`files/idwt.py` STEP 80; Appendix A §34). Items 4 and 6 follow from the standard noncommutative-geometry implications on the natural smooth subalgebra; the genuine residuals are the full infinite-dimensional $M_\infty$ (item 1), the KO-dimension obstruction (item 5), and the precise choice of smooth subalgebra (items 4, 6).
...
4. **Fredholm module structure. ✅ (internal, natural smooth subalgebra).** A spectral triple with compact resolvent and bounded commutators $[D,a]$ ($a\in\mathcal{A}$) yields a Fredholm module — $F=D|D|^{-1}$ with $[F,a]$ compact (standard, Connes). For the internal triple compact resolvent holds (item 2), and on the natural smooth subalgebra — $C^\infty$ functions of bounded gradient tensored with the constant matrix factor — $[D,a]=\gamma^\mu(\partial_\mu a)$ is bounded (the matrix factor is constant, hence commutes). So $[F,a]$ is compact and $(\mathcal{A},\mathcal{H},F)$ is a Fredholm module, defining the $K$-homology class. The only residual is fixing the smooth subalgebra precisely (a Fréchet/bounded-derivative completion), a definitional choice rather than an obstruction.

5. **KO-dimension. 🔶 (obstruction located).** The per-sector real dimensions mod 8 are $\{2,3,4,5,6,2\}$, summing to $22\equiv6\pmod 8$, but a single KO-dimension needs a global real structure $J$, which is obstructed: $d=4$ ($\mathbb{CP}^2$) is spin$^c$, not spin ($\mathbb{CP}^n$ is spin iff $n$ is odd), so it carries no canonical $J$; and $d=5$ ($S^5$) admits no charge-conjugation $C$ (the same fact that forbids $0\nu\beta\beta$). The spin sectors are $d\in\{2,3,5,6,10\}$. So the KO-dimension is not a single integer awaiting computation — the real structure is sector-dependent and structurally obstructed, tied to the chirality/Majorana content.

6. **Regularity. ✅ (natural smooth subalgebra).** A spectral triple is regular when $a$ and $[D,a]$ lie in $\bigcap_k\operatorname{Dom}(\delta^k)$, $\delta=[|D|,\cdot]$. For the smooth subalgebra of functions with all derivatives bounded and a Dirac operator $D$, the iterated commutators $\delta^k(a)$ are higher-derivative multiplication operators, all bounded, so $a,[D,a]\in\bigcap_k\operatorname{Dom}(\delta^k)$ — the standard regularity of the smooth function algebra. As with item 4, the only residual is fixing the smooth subalgebra precisely.
...
**Selection Principle T0.5.** A mode $(n,d)$ is a physical particle if and only if the pair $(n,d)$ is an element of $\Sigma_{\rm pairs}$ — the co-fixed-point set produced by the generation tower (the sector comb filtration generated by $n_s$). **Status: 🔶** semi-structural. The exclusion of non-co-fixed-point modes splits by level parity. The odd-level modes are excluded exactly: the $l=0$ seeds cannot reach an odd-$l$ mode through the $l=0\oplus l=2$ kernel at any order ($l$-parity, ⭐, Part 7 §1.2). The even-level modes are excluded by infinite-dimensional dephasing on timescale $1/m_{\mathrm{scale},d}$ (🔵), and this is the proven ceiling for that class: the $l=0$ seed-coupling overlap $J(n_r,s)=\Gamma(n_r+\tfrac32)/n_r!\cdot(s-1)^{n_r}/s^{n_r+3/2}$ has its only zero at $s=1$, while the kernel fixes the weight at $s=3/2$, so the coupling is strictly nonzero for every even-level mode and decays geometrically without vanishing (Appendix A §15; `files/idwt.py` STEP 34). An exact $l$-parity-style cut for the even-level class is therefore structurally impossible under the $(\xi\cdot\xi')^2$ kernel — only the quantitative dephasing rate is available.

**Scope — the three tiers of the spectrum.** T0.5 governs the *lattice*: the eleven additively generated modes — the photon ground state and the hockey-stick fermions (down 1, up 3, strange 4, $\nu_1$ 10, $e$ 13, $\nu_2$ 15, charm 20, $\nu_3$ 22, $\tau$ 23, $\mu$ 35), each a single $S(n,d)$ value or an additive simplex sum of seeds. This set closes under the hockey-stick operation with no reference to 16 or 72. The full observed spectrum adds two objects the additive tower does not generate: the two **product-form quark sites** — bottom $k_0 = n_s^2 = 16$ and top $n_{\rm top} = N_c\,n_s\,N_f = 72$, the heaviest quark of each type — whose first-principles origin is a separate open problem; and the **electroweak boson chain** $\{76, 81, 95\}$, which is the *same* additive $g$-rule seeded by the external top rather than by the seeds ($76 = g(5,72)$, $81 = g(6,76)$, $95 = g(15,81)$). There is one additive operation with two starting points — the seeds give the lattice, the top gives the bosons — and the chain builds strictly *upward* from the top, a directional fact that diagnoses the tower operation as additive rather than subtractive (Appendix A §43). The selection problem proper is therefore the eleven-mode lattice; the two product-form quarks are a distinct question, and the boson sector is one additive layer above the top.

The maximum within-sector return amplitude is bounded by the off-resonant Rabi sum $\sum_n (J/\Delta S)^2$ over all non-tower even modes, where $\Delta S > 0$ by spectral independence (⭐, the occupied $S$-values are sum-free, Part 1 §5). This sum converges in every sector: the worst case is $d=3$ at $\sum(J/\Delta S)^2 \approx 4\times10^{-4}$, bounding the coherent revival amplitude at $\lesssim 2\%$; the other sectors are smaller by $10^1$–$10^{13}$ (`files/idwt.py` STEP 49). Cross-sector revival is separately bounded to zero in the time-average by the incommensurability lemma (✅, Appendix A §15, STEP 40: 14 of 15 cross-sector scale ratios are irrational). Together these give a quantitative floor on the even-level dephasing: total return probability $\lesssim 4\times10^{-4}$. This is small but strictly nonzero, so the status remains 🔵 rather than ✅ — and an exact zero is not available from the sector dynamics: the $d>10$ vacuum dimensions are a dynamically closed bath (the kernel has no support beyond coordinate 10, so outer momentum conservation gives zero coupling width at all orders; `files/idwt.py` STEP 64, Appendix A §22). T0.5 is correct in its outcomes; the even-level selection is a quantitatively tight dephasing bound, not a proved exact theorem, and that bound is where the 🔶 sits. At kernel level the even-level selection is equivalent to the absolute stability of the electron, the u quark, and the neutrinos (Part 7 §1.2 stability equivalence, STEP 64), which gives it a sharp empirical witness: $\tau_e > 6.6\times10^{28}$ yr and the proton-decay bounds.
...
The d=6 row covers the electron and muon — both are d=6 CP³ sector excitations of Ψ∞, genuine 6D objects inhabiting six macroscopic spatial dimensions, whose activity in the three observable coordinates is what a d=3 observer measures.
...
> - **$d=7$ ($S^7$):** Hopf universality (T9b) is a consistency relation between active sector pairs, not an activation mechanism — it neither constructs nor forbids a $g_{77}$. Any activation route that treats the $d=6$ and $d=10$ bases symmetrically is excluded: their coupling rows are exactly equal ($v_6=v_{10}=1/2$, T9c, and rank-1 extends the equality to every cross-coupling), so such a route would equally activate $S^{11}$ over $\mathbb{CP}^5$, and $d=11$ is excluded both as a sector (Gegenbauer-subcritical, Rule B) and as a bare coupling row (a $v_{11}\neq0$ row links every sector mode, including e, u, and the neutrinos, to the $d=11$ continuum, against their absolute stability — Part 7 §1.2). A route keyed on the $\chi$-value itself — base $\chi$ in the consecutive run $\{N_c{-}1,N_c,N_c{+}1\}$ — fits both real activations ($d=3$ over $\chi=2$, $d=5$ over $\chi=3$), would fire at $d=7$ and not at $d=11$, and is not excluded by the above; see the open item.
>
> The only re-entry past the terminus is $d=10$, the Gegenbauer critical endpoint (Rule B), which takes $g_{10,10}=1/n_s$ by $\mu$–$\tau$ symmetry with $d=6$ (T9c). **Open item (Part 6):** the $d=7$ exclusion rests on an activation law underdetermined by the two real activation instances — "base coupling solves a kernel equation" fits both and excludes $d=7$; "base $\chi$ in the consecutive run" fits both and admits it. The arbitration is dynamical, and not a mean-field object: the §3.10 condensate self-consistency reduces to $2\lambda_d^{3/2}=g_{dd}$ with the dimension cancelling, so it is solvable for any coupling in any sector and does not distinguish $d=7$ (Hopf universality likewise writes a formal $g_{77}$ rather than forbidding it). The arbiter is the coupled $(\Psi_\infty,\{M_d\})$ fixed point — Part 6 Open Theorem A, the ground-state covariance operator whose spectral plateaux should fall at exactly $d\in D$ (Appendix A §3; `files/idwt.py` STEP 76). A corroborating constraint: an active $d=7$ would extend the matter quartet to five sectors, against the count $2n_s-4=n_s$ that independently certifies $n_s=4$ (Part 1 §3a). Until the arbitration closes, Rule A stands as a 🔶 selection rule.
...
**Top index from the sector set.** Once the sector set is established, the top index is the product of the Euler characteristics of the three Kähler matter sectors, $n_{\rm top}=\chi(\mathbb{CP}^2)\times\chi(\mathbb{CP}^3)\times\chi(\mathbb{CP}^5)=3\times4\times6=72$ (T15). This is its derivation. The value $72$ is not in the image of $S(n,d)$, so the top is a product-form site rather than a hockey-stick tower output, alongside the bottom beat $k_0=n_s^2=16$; among the six quarks these are the only two not given by a single $S(n,d)$ value, the other four being $1,3,4,20$ (T0.5).
...
| 7 | $0.507$ | supercritical; no activation route (Rule A; open residual, Part 6) | ✗ |
...
**Two qualitatively distinct types of excluded dimension.** The table makes visible a distinction that matters for the top-down picture of M_∞. Sectors $d\geq11$ are excluded at the most fundamental level: they are subcritical, meaning localization is geometrically impossible — modes cannot bind, they propagate and disperse into the infinite-dimensional bulk. Sectors $d\in\{7,8,9\}$ are supercritical (localization is in principle possible) but are excluded by coupling structure: the coupling-construction chain terminates at $d=6$ and none of the band acquires a self-coupling — $d=8$ is the Euler-characteristic gap ($\chi=5$, no $g_{88}$ fixed point), $d=9$ inherits that gap on its $S^1$-invariant block, and $d=7$ has no activation route (Rule A; the open residual is recorded in Part 6). These are qualitatively different failures. $d\geq11$ cannot host bound states at all. $d\in\{7,8,9\}$ could in principle support localized modes, but there is no mechanism in M_∞ to form the sector potential that would trap them.
...
A former correction multiplied each up-type mass by $(1-\varepsilon)^k$ with a per-quark exponent $k$. That exponent was a fit, not derived, so the correction is **removed**; the up-type masses are quoted bare and overshoot PDG 2024 (charm $1{,}284.9$ MeV, $+0.93\%$, $+2.6\sigma$; top $176{,}365$ MeV, $+2.20\%$, $+13\sigma$). The $\ell=2$ self-energy supplies the overshoot's sign but not its magnitude (Appendix A §45), so the overshoot is an open residue (🔶). See Part 2 §11. ($\varepsilon$ is retained as the derived scale of the separate $\nu_3$ closure, T11d.)
...
**Remaining open:** (i) CP phase — $\delta_{CP} = \pi + 2\theta_{13} = 197.11°$ derived via APS spectral flow (T8 🔶, Part 10); three technical gaps remain before 🔵 (spectral flow coefficient rigour, sign from T6 matrix, equivalence with the Fubini-Study curvature integral over coupling parameter space here). (ii) $G_N$ from $\infty$-dimensional gravity — $G_N=G_\infty/V_7$ where $V_7\approx7.74$ is fully derived; $V_{\rm vacuum}$ does not enter ($d>10$ is Ricci-flat in vacuum and T5 scattering states are not L²-normalizable); the single open item is $G_\infty$ from the spectral action scale $\Lambda$ on $M_\infty$ (Part 4 §3.12.2, Part 6). **Resolved:** m_ν₃ correction $\delta_{\nu_3}=\varepsilon\cdot g_{33}=1/35$ derived exactly (Part 2 §9d); $\Sigma m_\nu=60.393$ meV.
