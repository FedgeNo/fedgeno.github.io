...
- ~~2-loop QED matching for g₁~~ **CLOSED** — 2-loop RK4 (Machacek–Vaughn) computed numerically: residual improves from −1.8823% to −1.8810%, closing 0.0014 pp. Entire residual is the sin²θ_W structural gap (+0.37% from mode indices 76,81); no running order closes a structural prediction offset.
...
**δ_CP — mechanism identified, integral not yet evaluated.** T8 establishes the topological source: Δc₁ = c₁(CP³) − c₁(CP⁵) = −2, and the tree-level vacuum ξ_d⁰ = 0 forces δ_CP^(tree) = 0. A CP¹ estimate using the Fubini-Study geodesic distances in coupling space gives δ_CP ≈ 35.5°, underestimating the PDG NuFit value (~197°) by a factor of 5.5×, as expected for a CP¹ approximation of a geometry that lives in CP³ × CP⁵.

The natural candidate for the missing path is an adiabatic periodic evolution of the vacuum expectation values ξ_d(t) along the lepton-sector Hopf chain d=5→6→10→5, with period τ = 2π/ω_H where ω_H = √Tr(D²)/ℏ is the spectral-action scale already derived from the Higgs VEV. The adiabatic condition ω_H ≪ ω_d (Higgs scale far below all comb-filter frequencies) is satisfied by the existing IDWT spectrum, ensuring the Berry formula applies without non-adiabatic corrections.

What this proposal correctly provides: the loop topology (d=5→6→10→5), the topological class (Δc₁ = −2), the adiabatic justification, and the period τ from the spectral action — all without new parameters. What it does not yet provide: the explicit embedding of the path γ(t) in the Fubini-Study geometry of CP³ × CP⁵. The Berry phase is geometric (independent of traversal speed), so τ fixes the dynamics but not the area enclosed by γ in coupling space. The remaining computation is to embed the Hopf chain concretely in CP³ × CP⁵ — specifying how ξ_d(t) traces the moduli space — and then evaluate the surface integral of the curvature 2-form F = dA over the enclosed region. Once that embedding is specified, the integral is determined entirely by quantities already in the archive.
...
**g₂, α, G_F — now derived ✅.** The SU(2)_L coupling is determined by the CP²→CP¹ sector reduction: g₂ = Q_u √g_s = (2/3)√(2g₄₄/π²) = 0.65275 (PDG: 0.65270, +0.008%). The Fermi constant follows directly from the W propagator at q²≪mW²: G_F = g₂²/(4√2 mW²) = 1.1658×10⁻⁵ GeV⁻² (PDG: 1.1664×10⁻⁵, −0.05%). Both g₂ and mW are independently derived from IDWT — no Higgs VEV enters. The fiber-scale α gives 1/α = 131.8; running to m_Z requires α_s derived from IDWT couplings (open item — see §Open). W total width Γ_W = 2044 MeV (−2%), Z total width Γ_Z = 2444 MeV (−2.0%), muon lifetime τ_μ = 2.190×10⁻⁶ s (−0.3%). See Part 3 §0.7. g₁: 2-loop RK4 integration (Machacek–Vaughn β-functions; b₁=41/6 from IDWT particle content: N_c=3, N_gen=3, hypercharges from anomaly cancellation) gives g₁(m_Z) = 0.35068 (−1.8810%), closing 0.0014 pp beyond 1-loop. The entire residual is the sin²θ_W structural gap (+0.37% from mode indices 76,81) propagated via Δg₁/g₁ = Δ(sin²θ_W)/[2sin²θ_W(1−sin²θ_W)]. ✅ 2-loop item closed; residual is the sin²θ_W open item.
