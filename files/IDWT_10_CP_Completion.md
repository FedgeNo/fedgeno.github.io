...
Magnitude: $|V_{us}V_{cb}V_{ub}V_{cs}|=3.33\times10^{-5}$ (IDWT-derived CKM amplitudes). The framework-natural candidate is $\phi_{\rm link}=c_1(\mathbb{CP}^2)\cdot2\pi/k_0=N_c\cdot2\pi/n_s^2=67.5^\circ$ (the first-Chern-class holonomy per $k_0$-beat loop; $N_c=3$ and $k_0=n_s^2=16$ are IDWT constants), giving $J=3.078\times10^{-5}$ — within $0.07\%$ of PDG $3.08\times10^{-5}$. The $k_0$-loop count is the single un-forced piece.

**Status (updated 2026-06-24):** The leading-order vanishing $J_{\rm CKM}^{(\rm leading)}=0$ is ✅. The charge-selection ($V_{ub}$ localization), factorization, and real-beat theorems are derived sub-results (sub-🔵 within the overall 🔶). The hierarchy $J_{\rm CKM}\ll J_{\rm PMNS}$ is explained structurally and quantitatively at order-of-magnitude. Single open item: whether the $c_1$-flux holonomy per $k_0$-beat loop is forced by the mode structure — if so, $J$ closes to PDG at $0.07\%$. T8 remains 🔶 (independent of this sub-result).
