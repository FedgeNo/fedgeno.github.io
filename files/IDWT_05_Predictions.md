...
The structural predictions are the moment signs ($\mu_p > 0$, $\mu_n < 0$) and the $\mu_p/\mu_n$ ratio, which follow from the $udd$ colour-singlet projector. The absolute magnitudes of $\mu_p$ and $\mu_n$ are spin observables of the spinor sector, set by its Dirac spin-orbit structure — a computation that remains open: the spin-independent contact kernel cannot source a magnetic moment (`files/idwt.py` STEP 94), so no magnitude prediction is made here. The axial coupling is the geometric ratio $g_A = \sqrt{S(n_s+1,3)/S(n_s,3)} = 1.3229$ bare; the kernel-channel quench $P_L = 1/S(5,3)$ gives $g_A = 1.272480$ (🔵; PDG $1.2723 \pm 0.0023$, $+0.1\sigma$) — the quench is the spin-scalar $C_0$ share of the nucleon's equi-norm tower, sampled by construction per the Channel-Projection Lemma (Part 8 §10; `files/idwt.py` STEP 139), with the composite-localization premise the one open step.
