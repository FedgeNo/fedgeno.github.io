...
**The spectral flow per unit $\theta_{13}$** is carried entirely by the zero modes, not by a bulk crossing density. The $\mathcal{O}(k)$ mode tower on $\mathbb{CP}^n$ — the sector's own mode functions, labelled by the $\mathrm{SU}(n{+}1)$ isometry — has the exact spectrum (from the isometry Casimir; `files/idwt.py` STEP 132)

$$\lambda_l^2 = l\,(l+k+n), \qquad l = 0, 1, 2, \ldots$$

with zero modes exactly at the ground level $l=0$, a level of dimension $\binom{n+k}{n}$, and a strictly positive gap $\lambda_1^2 = k+n+1$ to the rest of the tower. For charge $k=1$ this gives exactly $n+1$ zero modes, gap-protected by $\lambda_1^2 = n+2 > 0$. (In bundle language the $l=0$ level is $H^0(\mathbb{CP}^n, \mathcal{O}(k))$; the count $n+1$ at $k=1$ equals $c_1(\mathbb{CP}^n)$. The canonical bundle $K_{\mathbb{CP}^n} = O(-n-1)$ has no global sections for $n \geq 1$ and is not the relevant object here.)

The flow coefficient follows by counting the sector's own modes, with no density estimate. Each charge-1 zero mode advances its phase by the base loop once per cycle — unit charge is the mode's grading under the loop $\mathrm{U}(1)$, the overall phase of the $n+1$ sector coordinates — so the combined phase of the $n+1$ zero modes, the object identified with the CP phase (§1.2), advances $n+1$ times the base. The gap $\lambda_1^2 = n+2$ keeps the zero-mode count stable for any perturbation amplitude below the gap (the coupling $v_0\sin\theta_{13}$, with $\|\Pi_n\|^2 = 1$ in the IDWT weighted $L^2$ norm, is far sub-gap), so the gapped tower contributes zero net flow and the rate is the exact zero-mode count:

$$\frac{d}{d\theta_{13}} \mathrm{sf}(D_{\mathbb{CP}^n}; \theta_{13}) = v_0 \times \dim H^0(\mathbb{CP}^n, O(1)) = v_0 \times (n+1) = v_0 \times c_1(\mathbb{CP}^n).$$
...
Magnitude and phase. The link phase $\phi_{\rm link}$ is a spectral-flow holonomy of the same determinant-line-bundle mechanism that fixes the lepton phase (§1.3): $\chi$ is the relative phase between the two Dirac eigenmodes $n=16,17$, so its rate is the anticanonical degree $c_1(\mathbb{CP}^n)=n+1$, not the matter-section degree $\mathcal{O}(n-1)$. For the up-sector that rate is $c_1(\mathbb{CP}^2)=N_c=3$ — the same value carried by the leading-order phase $c_1(\mathbb{CP}^2)-c_1(S^3)=3-0$ above — so a single-section reading with degree $k_u=2$ (which would give $45^\circ$) is excluded. The $16\leftrightarrow17$ link advances one node of the $k_0=n_s^2=16$ beat, giving $\phi_{\rm link}=c_1(\mathbb{CP}^2)\cdot2\pi/k_0=67.5^\circ$. This is the predicted unitarity-triangle angle $\gamma$; the PDG value is $\gamma\approx65.7^\circ$, a $+1.8^\circ$ ($\sim3\%$) discrepancy. The one-node step $f=1/k_0$ follows from the lock structure: the real-beat theorem locks $\chi$ at the beat nodes, the beat carries $k_0=16$ nodes per cycle (the established beat index, Appendix E §49), and the holonomy transport is homogeneous along the cycle (constant-curvature connection, so the phase is proportional to the arc fraction) — hence the phase accrued between adjacent locks, the quantity entering $\chi = \pi - \phi_{\rm link}$, is $1/k_0$ of the full loop. Robustness: varying $f\in\{1/15,1/16,1/17\}$ moves $\gamma$ across $\{72^\circ,67.5^\circ,63.5^\circ\}$ and changes $J$ by at most $\pm3\%$. The Jarlskog itself is $J=|V_{us}V_{cb}V_{ub}V_{cs}|\sin\gamma$; $|V_{us}|$ and $|V_{cb}|$ are IDWT-derived, and the $|V_{ub}|$ magnitude — the apex — is candidate-derived (🔶, `files/idwt.py` STEP 136) by the same conditional-Born counting law applied to the charge-selected hop: $R_{ub}^2 = n_u/S(n_s,3) = 3/20$, the pairing selected uniquely by data. This gives $|V_{ub}| = 3.67\times10^{-3}$ (PDG $(3.82\pm0.24)\times10^{-3}$, $-0.6\sigma$) and an all-IDWT Jarlskog $J = A^2\lambda^6\bar\eta = 3.06\times10^{-5}$ against the PDG global fit $J = (3.18\pm0.15)\times10^{-5}$ ($-0.8\sigma$); with the measured $|V_{ub}|$ instead, $J = 3.16\times10^{-5}$ ($-0.1\sigma$).

**Status.** The leading-order vanishing $J_{\rm CKM}^{(\rm leading)}=0$ is ✅. The charge-selection ($V_{ub}$ localization), the factorization and real-beat theorems, and the holonomy-degree identification — the link rate is $c_1(\mathbb{CP}^2)=3$, not the matter-section degree, with the $45^\circ$ reading excluded — are derived sub-results (sub-🔵 within the overall 🔶). The hierarchy $J_{\rm CKM}\ll J_{\rm PMNS}$ is explained structurally. The predicted phase $\gamma=c_1(\mathbb{CP}^2)\cdot2\pi/k_0=67.5^\circ$ agrees with PDG $65.7^\circ$ to $\sim3\%$, but its *magnitude* stays 🔶: the one-node step $f=1/k_0$ is derived from the node-lock structure (adjacent-lock transport, above), the $|V_{ub}|$ apex magnitude is not yet derived, and the result cannot exceed its parent T8 (🔶), on which the spectral-flow mechanism rests. T8 itself is unchanged.
...
- **CP-violating phase $\delta$ (APS spectral flow):** Structural derivation complete. $\delta_{CP} = \pi + 2\theta_{13} = 197.11^\circ$. Source: Chern class mismatch $\Delta c_1 = -2$ between $\mathbb{CP}^3$ and $\mathbb{CP}^5$, accumulated as APS spectral flow of the Dirac family on the charged-lepton sectors. Status remains 🔶 (see §4 status note): (i) the spectral flow coefficient is derived by exact zero-mode counting ($n+1$ charge-1 ground modes, gap-protected; §1.3, `files/idwt.py` STEP 132), with its residual — the T2-perturbation-to-loop identification — folded into the formalization item (vi); (ii) sign $\partial U_{e3}/\partial\theta_{13}|_{0^+}$ not explicitly computed from T6 coupling matrix; (iii) equivalence with Part 9 coupling-space integral not proved formally.
...
**Honest input accounting.** The table shows what follows from $N_c$, but $N_c$ is not the whole input set. Fully accounted, IDWT rests on **a primitive seed pair, one Euler-product index**, and **two dimensional inputs**:

- **Integer inputs** $\{n_d = 1,\ n_u = 3,\ n_{\rm top} = 72\}$. The primitive seeds are $n_d=1$ (ground seed) and $n_u = N_c = \chi(\mathbb{CP}^2) = 3$ (non-trivial seed, which drives the table). The top site $n_{\rm top} = 72$ is a **distinct** Euler-product index — expressible as $\chi(\mathbb{CP}^2)\,\chi(\mathbb{CP}^3)\,\chi(\mathbb{CP}^5) = 3\cdot4\cdot6$, but not forced from $N_c$ by any established selection principle (open; Part 6, Part 2 §11.7). The resonance beat $k_0 = n_s^2 = 16$ and the Higgs index $n_H = 95$ are likewise expressed through named operations whose selection rationale is still open.
- **Dimensional inputs** $m_e$ and $G_\infty = 4\pi G_N$. The mass unit $m_e$ sets the absolute mass scale; $G_\infty$ is a second dimensional input of a different kind — the curvature-per-unit-mass of $M_\infty$, not fixed by the combinatorics (Part 4 §3.12.4). The rate sector carries one further irreducible import: the quantum of action, entering as the one-quantum statement (a stable excitation carries unit conserved charge in anchored units — the number is 1, not fitted), provably not derivable from the classical axioms (One-Import Theorem, `files/idwt.py` STEP 135); rate ratios need no further input.
