...
### §13a. The $g$-rule transition graph on the mode set
...
**Level-form reading: the $g$-rule's $-1$ is the zero-join case of the join calculus (🔶, 2026-07-04).** Two-set test (both $n$ and the level $N = n-1$, the variable $S(n,d)$ actually counts over). The committed two-index composition rules split exactly: every fermion pair-sum — $n_s = n_d + n_u$, $n_e = n_{\nu_1} + n_u$, $n_\mu = n_c + n_{\nu_2}$, $n_\tau = n_{\nu_3} + n_d$ — is exact in index form, i.e. carries exactly $+1$ in level form ($N = N_1 + N_2 + 1$): one join quantum, the STEP 113/114 antisymmetric-pairing node. The one two-index $g$-combination, $n_H = g(n_{\nu_2}, n_Z) = n_{\nu_2} + n_Z - 1$, is exact in **level** form: $N_H = N_{\nu_2} + N_Z$ ($94 = 14 + 80$) — zero joins, pure level addition. IE edges ($\sum\varepsilon_i = 1$, e.g. $n_{\nu_3} = n_{\nu_1} + n_{\nu_2} - n_u$) are shift-invariant and exact in both columns. Reading: composite level $=$ sum of operand levels $+$ number of antisymmetric joins; fermionic composites pay one pairing node, the bosonic composite pays none (symmetric channel needs no antisymmetric node) — the $g$-rule is then not a separate rule but the joinless case of the same calculus, and its "$-1$" is pure $n \leftrightarrow N$ bookkeeping. Scope caveat: the W and Z steps use a sector *dimension* as an operand ($n + d - 1$) and are not covered by the two-index statement. Arithmetic verified exactly (integer/sympy); the join reading inherits the STEP 114 status.

**Extension to the full boson chain, and an honest correction (2026-07-04, same session).** Searching pair compositions $N = N_a + N_b + J$, $J \in \{0,1\}$, over the 13-mode pool (11 sub-72 modes $+$ photon $+$ top), each heavy boson has **exactly one** decomposition: $W = s \oplus t$ ($J{=}1$, i.e. $n_W = n_s + n_{\rm top} = 76$ index-exact), $Z = \nu_1 \oplus t$ ($J{=}0$, $N_Z = N_{\nu_1} + N_{\rm top} = 80$ level-exact), $H = \tau \oplus t$ ($J{=}1$, $n_H = n_\tau + n_{\rm top} = 95$) — with the equivalent joinless ladder route $H = t \oplus \nu_1 \oplus \nu_2$ ($N_H = 71 + 9 + 14 = 94$) via $Z$. The whole heavy chain is **top-anchored** in the calculus with light partners $\{s, \nu_1, \tau\}$. Controls (strict): 43\% of integers 36–130 are pair-reachable (mean 0.66 decompositions), and top-anchoring is *forced by magnitude* (the largest non-top pair reaches only $N=57$), so the individual hits are not statistically significant; the content is the unification (one calculus, unique decompositions, all committed forms recovered), not surprise. **Correction to the reading above:** $J$ does **not** sort by composite statistics — the four fermion sums all pay $J{=}1$ (consistent with the STEP 113/114 pairing node) and the g-rule instance pays $J{=}0$, but $W$ (a boson) pays $J{=}1$ and $H$ pays $J{=}1$ or $J{=}0$ depending on route; nor is $J$ explained by level parity alone (composite parities show no clean law). What survives: the covering, the uniqueness, the top-anchored chain with the $\nu$-ladder route for $Z, H$, and the $g$-rule-$-1$-as-$J{=}0$ instance. **The $J$-assignment rationale (resolved 2026-07-04, `files/idwt.py` STEP 142):** $J$ is the condensate-insertion count of the production, and its parity is fixed by the landing sector — the even-insertion rule (Part 3 §11). Matter landings carry exactly one insertion (the parity-flipping pairing node); $d{=}2$ landings carry an even number, because the $d{=}2$ variable has no condensate to supply a degree-1 vertex — the same structural fact as $m_\gamma = 0$. The apparent statistics-breaking "$W$ pays $J{=}1$" was an arithmetic face: the $s \oplus t$ pair decomposition is an identity of the value, while the committed $W$ production is the zero-insertion $g$-step landing on $d{=}2$, even as the rule requires. The pair-decomposition table above classifies faces; the insertion rule classifies productions. Consistency bonus: the next ladder rung $H \oplus \nu_3$ ($n = 116$, $m = 186$ GeV) is forbidden by the §46 spectral-closure headroom (20 GeV) — the ladder and the closure terminate the chain at the same place.
...
**Mode functions** are the flat harmonic eigenfunctions (Laguerre $\times$ Gaussian, §20 Part E, with closed-form norms) — not a proxy after all; they are the genuine self-binding modes. The spinor-harmonic / $S^d$ picture is their $\mathrm{SO}(d+1)$-covariant relabeling.
...
**(1) Three generations (⭐, given STEP 74e).** Label a deposit $(\alpha,\beta) = (\omega_2\text{ power},\,\omega_3\text{ power})$; its site $j = \alpha+\beta+2$ equals the physical sector $d$ (STEP 89). The per-sector channel count is the convolution of the two $U(k)$-channel counts — the coefficients of (1+x+x²)(1+x+x²+x³) = 1 + 2x + 3x² + 3x³ + 2x⁴ + x⁵ — so sectors $d = 2$..7 carry **1, 2, 3, 3, 2, 1** deposits. The peak of **3** at $d=4$ (up, charm, top) and $d=5$ (ν₁, ν₂, ν₃) is the three-generation count, capped by $k=2,3$ (the $U(k)$ nilpotency $\omega_2^3=\omega_3^4=0$). $d=3$ and $d=6$ carry 2 deposits each; the missing third members are exactly the two known exceptions — the bottom **beat** resonance ($k_0=16$, not a tower deposit, Part 7) and the **τ** (the $(\alpha,\beta)=(2,3)$ channel at site 7, forced to its physical sector $d=10$). The deposit structure separates the regular tower modes from the two exceptions automatically.

**(2) $\alpha$-ordering driver (🔶 motivated).** Within a sector the mode index $n$ increases with $\alpha = \#(\omega_2)$ (STEP 89). The clean case is the neutrino sector, where the index *is* the re-evaluated count: $\nu_1$ ($\alpha=0$) $= S(n_u,3) = S(3,3) = 10$ and $\nu_2$ ($\alpha=1$) $= S(n_u,4) = S(3,4) = 15$ — i.e. $d_{\rm eval} = 3+\alpha$, with $\omega_2$ the $d=4$ evaluation channel. Since $S(n,d)$ is strictly increasing in $d$ for $n \geq 2$ (here $n_u = 3$), each extra $\omega_2$ lands the seed on a strictly larger count, so $n$ rises with $\alpha$. (For $n=1$ the ground is degenerate, $S(1,d)=1$ for all $d$.) The other sectors order the same way but through tower constructions rather than a single $S(n_u, 3+\alpha)$; $\nu_3 = 22 = S(3,5)+1$ carries a tower additive offset, and the up/down/lepton indices come from the generation DAG. **Open:** a closed $n(\alpha,\beta)$ across all sectors — the within-bidegree $n$-assignment, the residual T0.5 piece. The between-degree count (three generations) follows from the $U(k)$ channel structure above.
