...
- **Scope of cleanup/error-checking:** When asked to look for errors, clean up language, or check for imported framework terminology, this means ALL files: every HTML file (articles/, visualizations/, index.html, etc.) as well as everything in the files/ directory. Do not limit searches to files/*.md only.
- **txt files in files/:** If a txt file exists in files/ and its content is worth turning into a website article, create the article HTML and add it to articles/index.html. If the content belongs in the existing documents instead, integrate it there. Either way, delete the txt file when done.
