...
|---|---|---|
| **Mass eigenvalue** | $F_{\rm mass} = m_{\rm scale,d} \times S(n,d)$ | Mass = sector scale × cumulative microstate count |
...
| $n_H$ | $n_u+n_c+n_{\rm top}=95$ | 🔶 empirical closure: $=3+20+72$; also $=n_d+n_e+n_Z=1+13+81$; dynamical derivation open |
...
$$k_0 = S(n_s,3)-S(2,3) = 20-4 = 16 \quad(\text{Vandermonde gap from absent }n{=}2\text{ mode})$$
...
The raw mass formula $m(n,d) = m_{\rm scale,d} \times S(n,d)$ reproduces the spectrum within most sectors, but in the **$d=4$ up-type quark sector** it overshoots, systematically and growing with generation: up +0.77%, charm +0.93%, top +2.20% vs PDG 2024. (The nominal $+2.6\sigma$/$+13\sigma$ these represent are against statistical errors only and are scheme-sensitive — heavy-quark masses differ by several percent between the pole and $\overline{\rm MS}$ schemes; §11.7.) Two pieces should be distinguished: the *generation-dependent* growth, which is accounted for by a derived $\ell=2$ self-energy with no free parameter (§11.1), and a residual quasi-*uniform* $\sim$0.6% offset, which is open. The up-type quark masses are quoted bare and the residual recorded honestly here.
...
The overshoot has a known sign and a known candidate source, though it is not yet derived to magnitude. The cross-sector kernel `(ξ_d · ξ_{d'})²` decomposes into an l=0 scalar part, which sets the sector mass scale, and an l=2 traceless tensor part, which contributes nothing to the scale but shifts each mode by a second-order self-energy. That self-energy was computed directly (`files/idwt.py` STEP 86): it is real, correctly signed (it pulls the masses down), and grows near-linearly with the mode index n — matching the observed pattern of an overshoot that grows with generation. Quantitatively, applying it with the *derived* prefactor $C=\varepsilon$ — no free parameter — collapses the generation-dependent scatter of the residuals from 1.43% to **0.20%**: the bare overshoots, spread from +0.77% (up) to +2.20% (top), become a quasi-*uniform* +0.5–0.7%. So the derived $\ell=2$ self-energy accounts for the *generation dependence* of the overshoot. What it does not remove is the residual uniform offset ($\sim$0.6%) — the $\ell=0$ (Level-1) sector-scale piece, still open. Two caveats keep this at 🔶 rather than closed: the self-energy's shape (exponent $p\approx0.96$) matches the combinatorial set $\{0,3,10\}$, which as a standalone correction does not land on PDG (only the fitted $\{0,7,16\}$ did), so it does not reproduce the measured masses exactly; and the uniform residual is itself underived. Status: 🔶 (generation-dependence derived via the $\ell=2$ self-energy; uniform residual and exact magnitude open).
...
The generation-*dependent* part of the overshoot is understood (§11.1): the $\ell=2$ second-order self-energy of the cross-sector kernel pulls the up-type masses down by an amount growing with generation, computed directly (`files/idwt.py` STEP 86) with the correct sign and near-linear growth, and with the *derived* prefactor $C=\varepsilon$ — no free parameter — it collapses the residual scatter from 1.43% to 0.20%. What remains open is the residual quasi-uniform $\sim$0.6% offset (the $\ell=0$ sector-scale piece), and the fact that the self-energy's $\{0,3,10\}$ shape does not land charm and top on PDG exactly. Neither is closed, so no correction is applied; both sit within the several-percent heavy-quark scheme spread (§11.7). This is the principal open item of the mass sector; the top is its sharpest case (§11.7).
...
Second is the mass. The bare mode $S(72,4)\,m_{\rm scale,4} = 176{,}365$ MeV sits $+2.20\%$ above the PDG 2024 pole mass ($172{,}570 \pm 290$ MeV). The nominal $+13\sigma$ this represents is measured against the *pole-mass statistical error alone* and overstates the tension: the top mass is scheme-dependent at the several-percent level — its pole and $\overline{\rm MS}$ definitions differ by $\sim$6% — so no single $\sigma$ count is meaningful across schemes. The honest statement is a $\sim$2% overshoot above the highest standard definition, not a $13\sigma$ falsification. Most of the structure is in fact accounted for: the derived $\ell=2$ self-energy (§11.1, `files/idwt.py` STEP 86) removes the *generation-dependent* part of the up-type overshoot with no free parameter (scatter 1.43%→0.20%), leaving a quasi-uniform $\sim$0.6% residual the top shares with up and charm. The scale-independent ratio is clean: $S(72,4)/S(20,4) = 137.26$ matches the conventional top/charm ratio to 1.25%.

So two things remain genuinely open at the top, and neither is a broken spectrum. The index $72$ is a product rather than a tower output, with no selecting condition yet found; and a uniform $\sim$0.6% offset — shared across the $d=4$ up-type, the $\ell=0$ sector-scale piece — is underived. The generation-*dependent* overshoot, by contrast, is captured by the derived $\ell=2$ self-energy. The top mass is therefore quoted bare ($\textbf{🔶}$): a $\sim$2% overshoot above the standard pole definition, with its generation-dependence accounted for and its uniform residual and index selection left open. It is an honest residue read with the scheme caveat above — not a derived agreement, and not a $13\sigma$ anomaly.
