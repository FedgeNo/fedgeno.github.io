...
**Mode functions χ_{n,d}(ξ) — not yet explicit.** The mass formula m(n,d) = S(n,d) × m_scale_d depends on mode functions that are never written down explicitly in these documents. Open: derive the explicit eigenfunctions of H_d = −Δ_Ξ + V_d(r) on CP^{d/2-1} or S^d with the confining potential V_d(r) = λ_d r²/(1+r²), in a form that permits computation of normalization factors and kernel matrix elements ⟨χ_{n_f,d_f}|K|χ_{n_i,d_i}⟩.
