...
**Why the residual is structurally protected, not merely small.** The second-order self-energy of an observable state through the CP³ hidden states decomposes by which coordinates the kernel touches, and only one of the three channels can affect fine structure. The pure-sector channel (kernel identity on the d=3 marginal) has an orbital-independent energy denominator by the Lemma 1 separability, so it produces a shift identical for every orbital — an exact common shift that cancels in every measurable difference by the §6.3 cancellation lemma. The d=3-only channel is diagonal in the sector index and has zero observable↔hidden matrix element (Lemma 2), contributing nothing. The mixed observable×sector channel is the only orbital-dependent one; it carries the contact factor L_6/a₀ at each of two vertices and is therefore the (L_6/a₀)² ≈ 7.1×10⁻¹⁰ residual above. Fine structure is protected by a selection rule — the pure-sector self-energy cancels, the d=3-only channel cannot reach the hidden states — and not only by the smallness of the contact factor (`files/idwt.py` STEP 77; Appendix A §33). The explicit value of the mixed channel requires the SO(3)⊂SU(4) embedding for the d=3⊂d=6 nesting (the open object of `files/idwt.py` STEP 66).
