...
**The spectral flow per unit $\theta_{13}$** is carried entirely by the zero-mode bundle, not by a bulk crossing density. The explicit $\mathcal{O}(k)$ holomorphic tower on $\mathbb{CP}^n$ has $\bar\partial$-Laplacian spectrum (exact, from the $\mathrm{SU}(n{+}1)$ Casimir; `files/idwt.py` STEP 132)

$$\lambda_l^2 = l\,(l+k+n), \qquad l = 0, 1, 2, \ldots$$

with zero modes exactly at $l=0$: $H^0(\mathbb{CP}^n, \mathcal{O}(k))$, dimension $\binom{n+k}{n}$, and a strictly positive gap $\lambda_1^2 = k+n+1$ to the rest of the tower. For the hyperplane bundle $\mathcal{O}(1)$ this gives exactly $n+1$ zero modes ($H^0(\mathbb{CP}^n, O(1)) \cong \mathbb{C}^{n+1}$; Hirzebruch–Riemann–Roch: $\chi(\mathbb{CP}^n, O(1)) = n+1 = c_1(\mathbb{CP}^n)$), gap-protected by $\lambda_1^2 = n+2 > 0$. Note: the canonical bundle $K_{\mathbb{CP}^n} = O(-n-1)$ has no global sections for $n \geq 1$ and is not the relevant object here.

The flow coefficient follows by counting, with no density estimate. Each $\mathcal{O}(1)$ zero mode carries unit charge under the loop $\mathrm{U}(1)$ (the central $\mathrm{U}(1)$ of $\mathrm{U}(n{+}1)$ acting on sections — the $\mathcal{O}(1)$ grading itself), so along one unit of the perturbation loop each of the $n+1$ zero modes advances its phase by the base holonomy once, and the determinant line of the zero-mode bundle — the object whose phase is the CP phase (§1.2) — winds $n+1$ times the base. The gap $\lambda_1^2 = n+2$ keeps the zero-mode count stable for any perturbation amplitude below the gap (the coupling $v_0\sin\theta_{13}$, with $\|\Pi_n\|^2 = 1$ in the IDWT weighted $L^2$ norm, is far sub-gap), so the gapped tower contributes zero net flow and the rate is the exact zero-mode count:
...
- **CP-violating phase $\delta$ (APS spectral flow):** Structural derivation complete. $\delta_{CP} = \pi + 2\theta_{13} = 197.11^\circ$. Source: Chern class mismatch $\Delta c_1 = -2$ between $\mathbb{CP}^3$ and $\mathbb{CP}^5$, accumulated as APS spectral flow of the Dirac family on the charged-lepton sectors. Status remains 🔶 (see §4 status note): (i) the spectral flow coefficient is derived by exact zero-mode counting ($\dim H^0(O(1)) = n+1 = c_1$, gap-protected; §1.3, `files/idwt.py` STEP 132), with its residual — the T2-perturbation-to-loop identification — folded into the determinant-line formalization item; (ii) sign $\partial U_{e3}/\partial\theta_{13}|_{0^+}$ not explicitly computed from T6 coupling matrix; (iii) equivalence with Part 9 coupling-space integral not proved formally.
