...
The observed quark CP violation, $J_{\mathrm{CKM}} \approx 3\times10^{-5}$, is therefore necessarily a subleading effect, coming from the holomorphic-versus-real mismatch in the kernel overlap between $\mathbb{CP}^2$ up-modes and $S^3$ down-modes. Writing the explicit $\mathbb{CP}^2$ holomorphic $\mathcal{O}(k)$ up-sections and real $S^3$ down-harmonics in shared Hopf coordinates (2026-06-24; `files/idwt.py` STEP 6; Appendix E §47) yields three derived results:
