...
Every mode index, sector assignment, and energy scale in this document is an output of the geometry of $\Psi_\infty$, not a quantity tuned to reproduce a measured mass. Three points make that concrete.

**The mode indices come from named combinatorial operations on the seeds.** From the seeds $n_d = 1$ and $n_u = 3$ (composite $n_s = 4$), the hockey-stick (Pascal) identity $S(n,d) = \binom{n+d-1}{d}$ fixes the simplex backbone exactly: $n_{\nu_1} = S(3,3) = 10$, $n_{\nu_2} = S(3,4) = 15$, $n_c = S(4,3) = 20$, and $n_\mu = S(4,4) = 35$. The last sits at the Pascal node $S(4,4) = S(4,3) + S(3,4)$, so the generation-2 lepton index is a combinatorial theorem rather than a coincidence. The composite indices enter through their own named operations: the additive and inclusion–exclusion edges ($n_e = n_{\nu_1} + n_u = 13$, $n_{\nu_3} = 22$, $n_\tau = 23$, $n_H = 95$), the Vandermonde $g$-rule for the weak bosons ($n_W = 76$, $n_Z = 81$), the bottom-quark resonance beat at $k_0 = n_s^2 = 16$, and the top's Euler product $n_{\rm top} = \chi(\mathbb{CP}^2)\chi(\mathbb{CP}^3)\chi(\mathbb{CP}^5) = 72$. No index is chosen to match a mass; the full chain is in §5 and Part 2 §6. The non-trivial seed $n_u = 3$ is itself fixed by $\chi(\mathbb{CP}^2) = N_c = 3$ (§3b), and the composite $n_s = n_u + n_d = 4$ is confirmed by $\chi(\mathbb{CP}^3) = 4$, independently of any mass data.
...
$\zeta_d(1) = d/(d-1)$ and $\zeta_d(0) = -d/2$ are exact for all sectors (Part 9 T13–T14, Pascal telescoping and heat kernel). All 15 particle masses follow from $m_{\rm scale,d} \times S(n,d)$ plus two corrections (geometric back-reaction for tau, beat resonance for b quark) and the $d=4$ up-type overshoot left uncorrected (the former GTC was removed, Part 2 §11). The six couplings $g_{dd}$ are fixed from seeds $\{n_d=1, n_u=3\}$ and composite $n_s = 4$; the masses introduce no further inputs beyond $m_e$.
