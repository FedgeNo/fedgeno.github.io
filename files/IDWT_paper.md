...
header-includes:
  - |
    ...
    \renewenvironment{abstract}{%
      ...
      \noindent\textbf{DOI:} \href{https://doi.org/10.5281/zenodo.21137675}{10.5281/zenodo.21137675}\par
    }
abstract: |
  We present Infinite-Dimensional Wave Theory (IDWT), a framework that provides a combinatorial derivation of the complete Standard Model mass spectrum, to high accuracy, from the primitive seed pair $\{n_d = 1,\, n_u = 3\}$ — whose composite $n_s = n_d + n_u = 4$ drives the fermion tower — together with the Euler-product index $n_{\rm top} = 72 = \chi(\mathbb{CP}^2)\chi(\mathbb{CP}^3)\chi(\mathbb{CP}^5)$ and a single dimensional reference $m_e$ (the electron mass, used only to convert dimensionless ratios to MeV). The mass of each particle is
...
This framework does not employ Higgs fields, Yukawa couplings, or spontaneous symmetry breaking to generate masses. The $W$ and $Z$ masses arise from the mode spectrum $m \propto S(n,d)$, like every other mass, not from a Higgs mechanism; the mass formula holds for all particles simultaneously.
...
That offset does not account for the whole residual, because the residual is not uniform within a sector: it grows with the mode index. In the $d=4$ up-type sector the bare overshoot climbs from $+0.77\%$ (up) to $+0.93\%$ (charm) to $+2.20\%$ (top). A scheme or scale shift cannot do this — it multiplies $m_{\mathrm{scale},d}$ by one common factor for every mode in sector $d$ (the kernel is rank-1), shifting them all equally, and cancels identically in the scale-free **in-sector ratio** $S(n_{\rm hi},d)/S(n_{\rm lo},d)$. The scale-free ratios confirm the point: the coloured quark sectors ($d=3$, $d=4$) carry in-sector residuals that grow with the level gap, whereas the boson ($Z/W$, $H/W$) and charged-lepton ($\mu/e$) ratios agree to a few parts in $10^4$. The overshoot is therefore $n$-dependent structure in the combinatorial count $S(n,d)$, confined to the quark sectors, not a uniform sector scale error — which is why the §7.1 confinement-binding correction is level-dependent (linear in $\langle k\rangle$) rather than a flat rescaling.
...
6. **The gravitational input $G_\infty$.** Gravity in IDWT is the back-reaction of $|\Psi_\infty|^2$ on the spacetime metric (Part 4 §3.12.2 [11]); the in-sector Green's function fixes the per-dimension coupling $G_d = G_\infty/[(d-2)S_{d-1}]$, equal to the measured $G_N$ at $d=3$. $G_\infty$ (equivalently $G_N$) is a second dimensional input alongside $m_e$ and is not derived from the sector combinatorics; the dimensionless combination $G_\infty m_e^2 \approx 2.2\times10^{-44}$ is simply an input, not a prediction. The genuinely open question is narrower — whether $G_\infty$ could instead be fixed geometrically, reducing the two dimensional inputs to one — which the present construction does not achieve.
7. **EW-scale self-consistency.** The RMS of the physical (confinement-corrected) mass spectrum gives $\sqrt{\operatorname{Tr}(D^2)}=245.5$ GeV, within $-0.33\%$ of the IDWT-derived EW scale $(\sqrt{2}\,G_F)^{-1/2}=246.3$ GeV; both are spectral quantities built from the same seeds, so the residual is a self-consistency offset (Part 9 T7 [11]), not an independent confirmation.

---

## 17. Conclusion

Starting from the seed pair $\{n_d=1,\, n_u=3\}$ — whose composite $n_s = n_d + n_u = 4$ is independently forced by the topological fixed point $S(4,4) = 35$ — together with the Euler-product index $n_{\rm top}=72=\chi(\mathbb{CP}^2)\chi(\mathbb{CP}^3)\chi(\mathbb{CP}^5)$, and the dimensional reference $m_e$, IDWT provides a combinatorial derivation of the complete Standard Model mass spectrum, the electroweak sector, CKM observables, and absolute neutrino masses to high accuracy. Five theorems ground the framework in spectral geometry, with complete proofs supplied above. The Completeness Theorem proves no additional particles exist within the framework — a consequence, not a claim. The $\tau$ back-reaction and $\nu_3$ closure corrections are derived from the kernel structure without additional inputs; the confinement-binding correction for the colour sectors ($d=3,4$) has a derived functional form, with its $d=4$ scale derived from the colour energy law and its $d=3$ scale anchored.
