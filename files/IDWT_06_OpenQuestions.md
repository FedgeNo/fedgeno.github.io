...
- ~~2-loop QED matching for g₁~~ **CLOSED** — 2-loop RK4 (Machacek–Vaughn) computed in open_computations.py: residual improves from −1.8823% to −1.8810%, closing 0.0014 pp. Entire residual is the sin²θ_W structural gap (+0.37% from mode indices 76,81); no running order closes a structural prediction offset.
...
**δ_CP — mechanism identified, integral not yet evaluated.** T8 establishes the topological source: Δc₁ = c₁(CP³) − c₁(CP⁵) = −2, and the tree-level vacuum ξ_d⁰ = 0 forces δ_CP^(tree) = 0. A CP¹ estimate using the Fubini-Study geodesic distances in coupling space (open_computations.py) gives δ_CP ≈ 35.5°, underestimating the PDG NuFit value (~197°) by a factor of 5.5×, as expected for a CP¹ approximation of a geometry that lives in CP³ × CP⁵.
