...
Non-$\Sigma_{\rm pairs}$ even-level modes are not BdG-unstable (their gaps also exceed their couplings) — they are excluded by dephasing, not exponential growth. The amplitude of an even-level non-member disperses into the infinite mode tower and does not return, because the cross-sector mass scales $m_{\rm scale,d}$ are incommensurate: every cross-sector ratio $m_{\rm scale,d}/m_{\rm scale,d'}$ is irrational except $m_{\rm scale,6}/m_{\rm scale,10} = 1$ (the μ–τ symmetric pair). This is a proved lemma — verified exactly by symbolic algebra over the algebraic definitions of the scales (`files/idwt.py` STEP 40; `claude/explore_scale_incommensurability.py`; Appendix A §15). Consequently no common revival period exists for any of the 14 pairs excluding the μ–τ pair. The incommensurability is now established; the remaining gap is that a rigorous demonstration would also require showing that the spectral measure of the reduced amplitude is continuous in the infinite-dimensional limit. The status 🔵 records this remaining step.
...
- **Even-level modes** ($n=3,5,7,\ldots$): nonzero off-diagonal coupling (EOM-derived from $L_{n,d}$, no node at $s=3/2$), giving permanent dephasing in the infinite-dimensional coupled system. The dephasing rate and coupling are EOM consequences; the incommensurability of cross-sector mass scales is now a proved lemma (Appendix A §15 ✅); the irreversibility of dephasing in the infinite-dimensional limit remains unproved. 🔵

**Structural note on the μ–τ pair.** The single commensurate pair is $d=6$ and $d=10$: $m_{\rm scale,6} = m_{\rm scale,10}$ exactly (Part 1 §5). This is the unique sector pair that can phase-lock — a new structural reading of why the tau, displaced to the non-adjacent $d=10$ sector, survives in the generation tower alongside the muon at $d=6$. The 14 remaining pairs are all incommensurate, so the no-revival argument applies to the full cross-sector dephasing dynamics for every non-member mode. ❓ (status: structural observation, not yet derived from EOM.)

Scripts: `files/idwt.py` STEP 30, STEP 34, STEP 35, STEP 36, STEP 40.
