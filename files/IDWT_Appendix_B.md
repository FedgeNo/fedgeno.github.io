...
Residue 2 contains only $\nu_1$ and $\nu_3$. The algebraic reason: $n_{\nu_3} - n_{\nu_1} = n_{\nu_2} - n_{\rm up} = 15 - 3 = 12 = 3\cdot n_s \equiv 0 \bmod n_s$. The second neutrino escapes to residue 3 because $S(n_{\rm up},4) - S(n_{\rm up},3) = q = 5 \equiv 1 \bmod 4$, not 0.
