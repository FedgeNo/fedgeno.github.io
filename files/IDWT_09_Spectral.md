...
| T0.5 | Co-fixed-point selection condition | 🔶 | Exact | Splits at $n_{\rm top}=72$: bottom regime ($n<72$) is the additive simplex tower (10 modes), with $k_0=16$ a derived Gegenbauer-endpoint overlay beat; top regime is the boson chain off the seed $n_{\rm top}=72$ ($W,Z$ by the $g$-rule, $H=95$ the additive closure); $n_{\rm top}=72$ is a seed input; even-level dephasing ceiling 🔵 is the proven status for the tower selection |
