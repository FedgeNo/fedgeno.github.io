...
| Bound within, gradient-free without: the gravitational gradient lives only in a source's bound dimensions | ✅ (nonperturbative on the connected branch; strong-field regime correctly excluded, §3.8) | §3.8 |
...
**The gravitational gradient lives only in the bound coordinates. ✅** Gravity is the curvature of $M_\infty$ sourced by the stress-energy $T$ (P5). If a particle's configuration is translation-invariant along $x_j$ — as it is in every coordinate it is uniform in — then $T$ is translation-invariant along $x_j$, and so is the geometry it sources: $\partial_j g_{\mu\nu} = 0$. A direction of uniformity of the source is a Killing direction of the sourced geometry, so there is no curvature gradient and no geodesic deviation along it. A uniform source exerts no pull along the direction it is uniform in. This is the gravitational counterpart of the kernel no-bath result — the wave is uniform in its outer coordinates, so outer translations are exact symmetries and the coupling width into that continuum is zero (Part 7 §1.2, `files/idwt.py` STEP 64). The argument is exact at the linearized level. The promotion of the source symmetry to the full nonlinear solution follows from uniqueness of the solution: if $\varphi_s$ is the translation flow of a uniform direction, the pullback $\varphi_s^*g$ solves the same Einstein system with the same source ($\varphi_s^*T = T$) and the same invariant boundary conditions, so wherever the boundary-value problem has a unique solution, $\varphi_s^*g = g$ for all $s$ — the uniform direction is a Killing direction of the full nonlinear geometry, not only of its linearization. This holds not only infinitesimally but nonperturbatively on the entire solution branch continuously connected to flat space. Scaling the source as $T(\lambda)=\lambda T_0$ and working in a $\varphi_s$-invariant harmonic gauge (so the static system is elliptic on weighted spaces encoding asymptotic flatness), the set of amplitudes for which $\varphi_s^* g(\lambda) = g(\lambda)$ is both open — wherever the gauge-fixed linearization is invertible, the implicit function theorem makes $\varphi_s^* g$ the same locally unique solution, so it equals $g$ — and closed — a limit of $\varphi_s$-invariant metrics is $\varphi_s$-invariant. It is therefore the whole interval $[0,\lambda^*)$ up to the first bifurcation $\lambda^*$, the first loss of linearized invertibility (`files/idwt.py` STEP 138). Every source to which the theorem applies lies far below $\lambda^*$: an elementary mode's Schwarzschild-radius-to-size ratio is $\sim 10^{-28}$ and galactic potentials are $\sim 10^{-6}$, so the connected-branch result covers all physical applications, including the dark-matter corollary and the finiteness of $G_N$. **✅** Beyond $\lambda^*$ — a strong-field regime no source in these applications reaches — uniqueness is neither claimed nor generally true: higher-dimensional static gravity has genuinely non-unique branches (black rings and their kin), so inheritance there would be an overclaim. That regime is correctly excluded, not left as a gap (🔶 only in the sense that it is deliberately not asserted).
