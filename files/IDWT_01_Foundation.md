...
- **Up-type overshoot (partly open):** the bare up-type masses run high, growing with generation (charm +0.93%, top +2.20%). A former $(1-\varepsilon)^k$ correction is removed — its exponent was a fit (Part 2 §11.3). The derived $\ell=2$ kernel self-energy accounts for the generation-*dependence* with no free parameter (residual scatter $\to$0.2%; STEP 86), leaving a quasi-uniform $\sim$0.6% offset that is open and sits within the several-percent heavy-quark scheme spread (Part 2 §11.7). Charm and top are quoted bare.
...
‡ Charm and top are quoted bare ($m_{\rm scale,4} \times S(n,4)$) and overshoot PDG 2024 by +0.93% and +2.20% — open residues (🔶). The nominal $+2.6\sigma$/$+13\sigma$ are against statistical errors only and are scheme-sensitive: heavy-quark masses differ by several percent between the pole and $\overline{\rm MS}$ schemes, so the top's $+2.2\%$ is a $\sim$2% overshoot above the standard pole mass, not a $13\sigma$ falsification (Part 2 §11.7). A former $(1-\varepsilon)^k$ correction (the "Generation Tower Correction," Part 2 §11) landed them on PDG, but its per-quark exponent was a fit, so it has been removed. The derived $\ell=2$ self-energy (`files/idwt.py` STEP 86) does account for the generation-*dependence* with no free parameter (residual scatter $\to$0.2%), leaving a quasi-uniform $\sim$0.6% offset that is open; the scale-independent ratio $S(72,4)/S(20,4)=137.26$ matches the conventional top/charm ratio to 1.25%. The up quark sits at +0.77%, within the light-quark error.
