...
**The charged-lepton tower shows the parallel third-generation displacement. 🔶** Because $n_{\rm top} = 72$ is off the tower, it cannot serve as an operand either. Generations 1–2 fix the charged-lepton index by adding the same-generation up-type quark ($n_e = n_{\nu_1}+n_u$, $n_\mu = n_{\nu_2}+n_c$), but the third would be $n_\tau = n_{\nu_3}+n_{\rm top}=94$, so $\tau$ is instead displaced by one ground quantum to $n_\tau = n_{\nu_3}+n_d = 23$ (Part 2 §3). The bottom beat and the $\tau$ displacement are two faces of one fact — a third-generation mode whose natural tower continuation is a product-form anchor ($k_0=16$ for the bottom, $n_{\rm top}=72$ for $\tau$) cannot follow the simplex tower and is reset by the ground quantum $n_d$. The neutrino value $n_{\nu_3} = S(3,5)+n_d = 22$ is a different case: it is not an index-addition (no two smaller tower indices sum to $22$) but the inclusion-exclusion mode $n_{\nu_1}+n_{\nu_2}-n_u$, equal to $S(3,5)+n_d$ only through $n_s=n_u+n_d$ (`idwt.py` STEP 112).
