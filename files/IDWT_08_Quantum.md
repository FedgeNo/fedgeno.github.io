...
**$\mathbb{CP}^2$ and colour:** $\mathbb{CP}^2$ requires spin^c rather than spin. The spin^c connection carries an auxiliary $U(1)$ bundle, geometrically identified with $U(1)_Y$ (hypercharge). Every eigenspace of $D^c_{\mathbb{CP}^2}$ is an $SU(3)$ representation from the $\mathbb{CP}^2$ isometry, providing the geometric basis for colour charge. The colour states themselves emerge from the Dirac index and the isometry action (§8); promoting the spin$^c$ $U(1)$ bundle to a local $SU(3)$ *gauge* symmetry is not required — IDWT has no Yang-Mills action and colour acts through the $d=4$ kernel contact term, so the gauge promotion belongs to the mediator framework IDWT does not adopt (§8).
...
**Note ($\Xi_d$ vs $\mathbb{CP}^n$) — correspondence proved (🔶). ✓** The Atiyah-Singer / HRR computation above is carried out for compact $\mathbb{CP}^2$. The non-compact sector space $\Xi_4$ has the same index by the following three-step argument.

*(1) Fredholm property.* The confining potential $V_4(r)=\lambda_4 r^2\to\infty$ gives $D^{c\dagger}D^c+V$ compact resolvent; $D^c$ is therefore Fredholm on $L^2(\Xi_4)$ with a well-defined integer index.

*(2) Localization of zero modes.* Let $\Psi$ be any $L^2$-zero mode, $D^c\Psi=0$. The Weitzenbock formula in the large-$r$ (flat-metric, low-curvature) region outside the $\mathbb{CP}^2$ ball gives $D^{c\dagger}D^c=\nabla^*\nabla+R(x)/4+F(x)$, with $R(x)\to 0$ and $F(x)\to 0$ as $r\to\infty$ (curvature concentrated near $r=0$ by the $\mathbb{CP}^2$ local model). The effective mass $M_{\rm eff}(r)=\sqrt{V_4(r)}=\sqrt{\lambda_4}\,r$ grows without bound, so for large $r$ the solution satisfies $|\Psi(x)|\le C\exp(-\sqrt{\lambda_4}\,r^2/2)$ (Carleman-type estimate for the massive Dirac equation). Every $L^2$-zero mode is therefore exponentially concentrated in the $\mathbb{CP}^2$ local-model region near $r=0$.

*(3) Reduction to compact model and Kodaira vanishing.* Since zero modes are supported within the $\mathbb{CP}^2$ region, the zero-mode problem on $\Xi_4$ reduces to that on $\mathbb{CP}^2$. By the Dolbeault isomorphism, $\dim\ker D^c = \dim H^0(\mathbb{CP}^2,\mathcal{O}(1)) = 3$ (degree-1 homogeneous polynomials in three variables). Anti-zero modes vanish: $\dim\ker D^{c\dagger}=\dim H^2(\mathbb{CP}^2,\mathcal{O}(1))=0$ by Kodaira–Nakano vanishing ($\mathcal{O}(1)$ is ample). Therefore $\mathrm{ind}(D^c\otimes\mathcal{O}(1),\Xi_4)=3-0=3$. The argument generalises to all even-$d$ sectors: $\mathrm{ind}(D^c\otimes\mathcal{O}(1),\Xi_{2m})=\dim H^0(\mathbb{CP}^m,\mathcal{O}(1))=m+1$ (see §2.4 table). One formal step remains open: the APS (Atiyah–Patodi–Singer) eta-invariant on the transition boundary $S^{2m-1}_R$ as $R\to\infty$ should be shown to vanish (expected because $D^c$ on $S^{2m-1}_R$ acquires effective mass $\sim\sqrt{\lambda}R\to\infty$, driving the eta function to zero). This technicality does not affect the result but is not yet written out within IDWT.
...
|--------|----------|------------|-------|---------|
| $d=2$ | $\mathbb{CP}^1$ | 1 | $C(2,1) = 2$ | 2 boson polarizations (photon/$W^\pm$); boson sector — index counts polarizations, not fermionic zero modes |
...
*Not an open item:* a local $SU(3)$ **gauge** promotion of the spin$^c$ $U(1)$ bundle. IDWT has no Yang-Mills action; $SU(3)$ is the $\mathbb{CP}^2$ isometry and colour acts through the $d=4$ kernel contact term (§8), so this belongs to the mediator framework IDWT does not adopt.
...
**Only colour-neutral configurations have zero colour-energy.** This follows from $E_{\rm conf} = \lambda_c|\vec{N}|$ applied to the specific $SU(3)$ colour vectors. **Note:** this establishes colour-neutrality as the geometric stability criterion — the IDWT-native form of confinement, an admissibility/superselection rule on colour charge (Route A). It is deliberately *not* the QCD dynamical picture: IDWT carries no propagating colour field and no Yang-Mills kinetic term (§0.2/§0.6), so flux-tube formation, asymptotic freedom, and the running of $\alpha_s$ are not features IDWT possesses or owes. They are constructs of the propagating-gauge-field framework IDWT does not adopt, and their absence here is structural, not a missing derivation.
...
**What remains genuinely open (IDWT-native):** (1) Deriving $\lambda_c$ from the IDWT action ($\lambda_c$ is free until connected to $m_{\rm scale,4}$ or $g_{3,4}$; note $\lambda_c$ is distinct from $\varepsilon = 1/(280\sqrt{7})$) — partially answered by the confinement-scale match below; and the single native dynamical question, why $E_{\rm conf}$ is linear in $|\vec{N}|$ (confining) rather than quadratic (screening), which reduces to the co-fixed-point / admissible-asymptotic-mode selection (Open Theorem A). **Not open, because not applicable:** 'promoting' the $\mathrm{spin}^c$ $U(1)$ bundle to a full $SU(3)$ *local gauge* symmetry, and the QCD dynamical apparatus (flux-tube formation, asymptotic freedom, a fundamental string tension). IDWT has no Yang-Mills action and no propagating colour field: $SU(3)$ enters as the $\mathbb{CP}^2$ isometry acting on colour space (§8), colour acts through the $d=4$ kernel contact term, and confinement is the colour-neutrality selection rule. Those gauge/flux-tube constructs belong to the mediator framework IDWT does not adopt; they are not derivations IDWT owes.

**Confinement scale (🔵 — numerical match, mechanism open).** Item (1) is partially answered: the derived colour scale $\lambda_c = N_c^2 f_\pi/2 = 423$ MeV fixes the confinement *scale* through $\sigma = \lambda_c^2$, where $\sigma$ is the measured QCD string tension. This single derived scale reproduces three independent confinement observables: the string tension $\sqrt{\sigma} = 423$ MeV (vs. $421$ MeV from Regge slopes, $+0.5\%$; $440$ MeV lattice, $-3.8\%$), the Regge slope $\alpha' = 1/(2\pi\sigma) = 0.889\ \mathrm{GeV}^{-2}$ (vs. $0.88$–$0.93$ measured), and the pure-gauge deconfinement temperature $T_c = 0.629\sqrt{\sigma} = 266$ MeV (vs. $260$–$270$ MeV lattice). This is a numerical match of a *derived scale* to measured confinement data — **not** a derivation of the confinement mechanism. The one native dynamical question that remains is why $E_{\rm conf}$ is linear in $|\vec{N}|$ (hence confining rather than screening), which reduces to the co-fixed-point selection (Open Theorem A); the QCD dynamical constructs (flux tubes, asymptotic freedom, a fundamental string tension) are not IDWT features, so their absence is structural rather than an open item (`files/idwt.py` STEP 63a).
...
**The isometry is the colour structure — no gauging required.** The $d=4$ sector geometry is $\mathbb{CP}^2 = SU(3)/U(2)$, whose isometry group is $SU(3)/\mathbb{Z}_3$. $SU(3)$ therefore acts on the $d=4$ colour space as a geometric isometry, and this *is* IDWT's colour symmetry. It is deliberately not promoted to a 3+1D local Yang-Mills gauge symmetry: IDWT has no Yang-Mills kinetic term and no propagating colour field (§0.2/§0.6, Part 3), so 'gauging' $SU(3)$ into a mediator gauge theory is foreign machinery the framework neither adopts nor needs. Colour acts through the $d=4$ kernel contact term, with the isometry supplying the three colour states ($\chi(\mathbb{CP}^2)=3$) and the colour-neutrality selection rule (§5).

**Precise status:** $SU(3)$ as the $\mathbb{CP}^2$ isometry acting on $d=4$ sector modes is established geometrically (Aut($\mathbb{C}^3,\Omega)=SU(3)$, verified above). A local $SU(3)$ gauge symmetry of a Yang-Mills action is not part of IDWT — its absence is a structural feature of the no-mediator framework, not an open derivation IDWT owes.
...
where $P_L$ is the small-component probability and $\tfrac{1}{3}$ is the angular average of the opposite-parity ($l=1$) small component. This is not an orbital $l=1$ admixture — none arises from the scalar contact kernel at any order (`files/idwt.py` STEP 94); the kernel sets only the spatial scale of the well. Taking $P_L = 1/S(5,3)$ — one small-component unit relative to the $S(5,3)$ states of the destination level — gives $q = 101/105$ and $g_A = \sqrt{7/4}\cdot(101/105) = 1.2725$, within the PDG uncertainty (`files/idwt.py` STEP 95). The identification of $P_L = 1/S(5,3)$ is completed in three steps below. The sector $S^3$ Dirac operator supplies two ingredients: the cumulative per-sign degeneracy up to level $k$ equals $2\,S(k{+}1,3)$ (so $S(4,3)=20$ and $S(5,3)=35$ are forced Dirac counts), and the exact small/large form $\int f^2/\int g^2=(E-m)/(E+m)$. These reduce the residual to the single ratio $E/m=S(5,3)/(S(5,3)-2)=35/33$. A third structural input now identifies the $-2$ (proved 2026-06-24): the kernel $(\xi\cdot\xi')^2$ on $S^3$ has Chebyshev-U expansion $t^2=(C_2^1(t)+C_0^1(t))/4$, so the sector potential couples $S^3$ Dirac levels only with $\Delta k=0$ or $\Delta k=2$. The nucleon's source level $k=2$ (per-sign count $S(4,3)=20$) therefore connects down by $\Delta k=-2$ to $k=0$ (count $C_0=2$ per-sign) and up by $\Delta k=+2$ to the destination $k=4$ ($S(5,3)=35$ per-sign): the small component lives at $k=0$ because the Gegenbauer coupling topology forces it. The "$-2$" is accordingly $C_0=2$ by structure — not a free parameter. The equi-norm *denominator* — why the tower carries $C_K=70$ states — is the **addition theorem** for a maximally localized (delta-like) large component on the homogeneous space $S^3=SU(2)$: by group invariance and Schur's lemma the per-level projector diagonal $\operatorname{Tr}P_k(x,x)=\deg_k/\mathrm{Vol}(S^3)$ is constant in position, so a point-like source carries equal norm per state across the full $k=0..4$ tower (per-sign cumulative $2\,S(5,3)=70$, the $g_A$ destination level) — the same point-like character the size program assigns to every elementary mode (`files/idwt.py` STEP 121). The *numerator* — that the small component entering the quench is the $k=0$ multiplet alone ($C_0=2$) — is now data-discriminated among the framework-natural readings (`files/idwt.py` STEP 137). The two senses of the small component are computed exactly on the tower at the committed scale ($\omega/m = 68/3267$, ground $E_0/m = 35/33$ exact): the *spectral* reading — the per-level Dirac lower component of the equi-norm tower, $(f/g)^2_k = (E_k-m)/(E_k+m)$ averaged with degeneracies — gives small/large$^2 = 0.0788$, hence $g_A = 1.194$, excluded at $-34\sigma$; the *channel* reading — the kernel-channel $k=0$ share of the equi-norm tower, $2/68 = 1/34$ — gives $g_A = 1.27248$ ($+0.1\sigma$), confirmed. With the competing assignments $1/22$ ($-12.9\sigma$) and $13/35$ ($-263\sigma$) already excluded, the channel reading is the unique data-consistent identification. The mechanism frame: the weak transition operator is a kernel-channel object (the chiral electroweak channel, `files/idwt.py` STEP 107), so the axial response samples the kernel's $C_0 \oplus C_2$ angular channel, and the spin-scalar $C_0$ share of the equi-norm tower is $\deg_0/(2S(5,3)) = 1/35$ — exact given the addition theorem. The channel projection is proved by the Channel-Projection Lemma (`files/idwt.py` STEP 139): every IDWT observable is a function of mode frequencies and kernel matrix elements only — the action has exactly two $\Psi$-terms, the kinetic + mean-field operator is diagonal in the mode basis (phases only), all amplitude dynamics run through the kernel's exact $C_0 \oplus C_2$ channel, and detection is itself the P4 density–density coupling. A decomposition defined by any other operator (the free-Dirac $\sigma\cdot p$ split) enters an observable only through its channel overlap, so the Gamow–Teller matrix element samples the kernel-channel content by construction — the a priori form of the $-34\sigma$ exclusion. The quench derivation is therefore complete modulo one premise, the composite (nucleon) maximal localization behind the equi-norm step (the size-program thesis, `files/idwt.py` STEP 121): $g_A = \sqrt{7/4}\,(101/105) = 1.272480$ (🔵; PDG $1.2723 \pm 0.0023$, $+0.1\sigma$).

**Magnetic moment magnitudes — candidate, no fitted parameters (🔶, `files/idwt.py` STEP 140).** The same structure now yields candidate magnitudes. Baseline (exact): the colour-singlet's permutation symmetry shares the composite mass equally over the three legs, $m_{q,\rm eff} = m_p/N_c$, which with the spin-flavor projector gives the equipartition values $\mu_p = 3$, $\mu_n = -2$ (isovector $5$, isoscalar $1$). Quench: by the Channel-Projection Lemma the moments are kernel-channel matrix elements of the $d=3$ spinor structure, and the only available reduction factors are the already-derived $m/E = 33/35$ and $q_{GT} = 101/105$ — there is no number to tune. A declared 28-candidate grid (Appendix A §9) selects, with clear separation, isovector $= 5\,(m/E)$ and isoscalar $= (m/E)^2$:
...
with $\mu_p - \mu_n = 33/7$ ($+0.18\%$) and ratio $-1.46479$ ($+0.34\%$). The isovector power is derived: the isovector $M1$ is a single-quark spin flip, the leading Dirac moment $\sim Q/2E$, which carries exactly one relativistic reduction $m/E$. The isoscalar second power is not: the SU(6) baseline isoscalar $\mu_p+\mu_n = 1$ is nonzero, so it carries a spin-magnetic part that power-counting assigns one factor, and a pure $(m/E)^2$ needs the spin-isoscalar to cancel — which SU(6) does not make exact. So the residual is narrowed to the isoscalar power alone (data-selected; the uniform one-power reading gives $\mu_p+1.28\%$, $\mu_n-1.43\%$, which the data disfavours). A candidate structural mechanism (not yet a derivation): the pairwise kernel is symmetric under leg exchange, so the exchange-odd isovector current's coherent two-leg piece cancels, leaving a one-leg ($C_0$) readout, while the exchange-even isoscalar current survives only in the coherent two-leg channel, picking up the same $P_L=1/35$ reduction on each of two legs — $(m/E)^2$, not a new number. The isoscalar data ratio matches $(m/E)^2$ to $-1.0\%$, plain $m/E$ only to $-6.7\%$, and the standard relativistic-quark-model alternative (single-quark orbital compensation, $(1+m/E)/2$) only to $-9.4\%$ — the data reject the textbook mechanism in favour of the coherent two-leg reading. What remains unproved is a selection rule excluding the mixed spin-scalar$\times$density cross term in the leg-symmetric channel (without it the generic result would be a mixture, not the pure square) — the same kind of addition-theorem computation that produced $P_L=1/35$, not yet carried out for this pairwise object. Still 🔶. The same structure sets $\mu_p$, $\mu_n$ and the $^3S_1$–$^3D_1$ tensor. The derived $g_A = 1.2725$ enters the neutron beta-decay lifetime: with $G_F$ (`files/idwt.py` STEP 5), $V_{ud}$ (CKM, Part 3 §12), and the Fermi integral $f = 1.6887$ (the $n$–$p$ Q-value, not derived natively), the Sargent law gives $\tau_n \approx 918$ s vs PDG $878.4$ s ($+4.5\%$); the residual is carried by $f$ (`files/idwt.py` STEP 109).
...
**The projective orbit construction.** A Hermitian $4\times4$ matrix generates a conserved Hamiltonian flow on the local projective model $(\mathbb{CP}^3,\omega_{\rm FS})$,

$$H([z])=\frac{z^\dagger Mz}{z^\dagger z},\qquad [z(t)]=[e^{-iMt}z(0)].$$

The brackets matter: $z$ and $cz$ represent the same point for every nonzero complex $c$, so a common phase cannot be used as an observable angle. A gauge-invariant aligned-plane construction uses the traceless generator

$$M_L=\omega\begin{pmatrix}\tfrac14&L&0&0\\L&\tfrac14&0&0\\0&0&-\tfrac34&0\\0&0&0&\tfrac14\end{pmatrix},\qquad z(0)=\left(A,0,\frac{B}{\sqrt2},\frac{B}{\sqrt2}\right),\qquad A,B>0,\quad A^2+B^2=1.$$

Writing $\theta=\omega t$, its exact flow is $z_1=Ae^{-i\theta/4}\cos(L\theta)$, $z_2=-iAe^{-i\theta/4}\sin(L\theta)$, $z_3=(B/\sqrt2)e^{3i\theta/4}$, and $z_4=(B/\sqrt2)e^{-i\theta/4}$. At $\theta=2\pi$ every component has acquired the same phase, so the projective orbit is closed. Both

$$r([z])=\frac{|z_1|}{\sqrt{|z_1|^2+|z_2|^2}},\qquad e^{i\phi([z])}=\frac{z_3\bar z_4}{|z_3z_4|}$$

are unchanged by $z\mapsto cz$. Therefore the chosen observer-plane map is exactly

$$X+iY=Rr([z])e^{i\phi([z])}=R|\cos(L\theta)|e^{i\theta}.$$
...
Normalize $\omega_{\rm FS}$ by $(2\pi)^{-1}\int_{\mathbb{CP}^1}\omega_{\rm FS}=1$. The prequantum connection on $\mathcal O(L)$ has curvature $-iL\omega_{\rm FS}$, the level-$L$ moment map is $L\mu_G$, and the holomorphic lift is

$$\widehat G_L=\sum_{j=1}^4g_jz_j\frac{\partial}{\partial z_j}.$$

Thus $\widehat G_Lz^a=(a\cdot g)z^a$. A central shift $G\mapsto G+cI$ adds the same $cL$ to every degree-$L$ state, hence only a common projective phase. This is the explicit curvature-to-operator bridge; there is no ordering choice in the induced action.
...
This is the full displayed matrix, not only its amplitude block. The scalar $I/4$ fixes an energy zero separately on each four-state subspace; it is not the lift of one shell-independent fundamental central shift, which would contribute $cLI$. The amplitude exchange and relative-phase clock use the same group parameter $\theta=\omega t$, so their frequency lock is representation-theoretic. No shell-dependent interaction strength is fitted. The electron's coupling charge remains $q=-e$; $L$ is the first Chern number of the magnetic background and the degree of its state space, not an electron charge $Le$. (`files/idwt.py` STEP 130.)

**Uniqueness at the stated premises.** Let a shell-independent diagonal action have fundamental weights $(g_1,g_2,g_3,g_4)$. Require the disjoint amplitude/clock splitting above, the amplitude pure powers $z_1^L,z_2^L$, the reference $z_4^L$, and the primitive adjacent clock state $z_3z_4^{L-1}$. Subtracting the reference weight gives

$$g_1-g_4=1,\qquad g_2-g_4=-1,\qquad g_3-g_4=-1.$$

The constraint matrix has rank three and its nullspace is the central direction $(1,1,1,1)$. Hence $G=cI+\operatorname{diag}(1,-1,-1,0)$ is unique up to a projectively invisible shift, reversal of orientation, and coordinate relabelling. Within the clock plane, $z_3^kz_4^{L-k}$ has relative weight $-k$, so primitive unit winding uniquely selects $k=1$; larger $k$ are clock harmonics. This is a classification under the stated plane-splitting premises, not a claim about every possible non-diagonal decomposition.

The full unitary-equivalence class is $UGU^\dagger$, not one displayed coordinate matrix. The eigenvalue multiplicities of $G$ are $(2,1,1)$, giving stabilizer $U(2)\times U(1)\times U(1)$ and a ten-real-dimensional conjugacy orbit. After subtracting its trace, $G$ lies in $\mathfrak{su}(4)$; using $SU(4)\simeq\operatorname{Spin}(6)$ identifies this with the six-dimensional rotation orbit when the four coordinates carry the corresponding spinor action. An isolated isotropic nucleus can then preserve the degeneracy while preparation or apparatus alignment selects a representative. Without that physical action, only unitary equivalence is proved. In the locked extremal construction, a Chern sector $k\geq0$ produces $2k$ lobes under the primitive clock, so identifying it with the angular family having $2L$ lobes forces $k=L$; signed $k$ gives $|k|=L$ with opposite orientation.

**Observer-map classification.** Let $q=|z_1|^2/(|z_1|^2+|z_2|^2)$ and $e^{i\phi}=z_3\bar z_4/|z_3z_4|$. Projective invariance and rotational covariance give $X+iY=Rh(q)e^{\pm i\phi}$ for a possibly complex response $h$. Reflection symmetry across the aligned apparatus axis removes an occupation-dependent angular shear and makes $h=f$ real, giving

$$X+iY=Rf(q)e^{\pm i\phi}.$$

If $f(0)=0$ and $f$ is strictly increasing, the maxima and zeros of $q=\cos^2(L\theta)$ are preserved, so the map has exactly $2L$ lobes independently of the detailed response. The additional intensity premise $(X^2+Y^2)/R^2=q$ uniquely selects $f(q)=\sqrt q$ and hence the displayed curve. The primitive phase is unique up to orientation, but the radial intensity premise is detector physics. Uniform group time $d\theta/(2\pi)$ gives a canonical pushforward occupation measure; its amplitude fraction has density $[\pi\sqrt{q(1-q)}]^{-1}$ and the displayed radius has density $2/[\pi\sqrt{R^2-r^2}]$. The third-coordinate choice remains non-unique: projective symmetry permits inequivalent functions $Z=D(q,\phi)$ and does not select the visualization's $\sqrt{1-q}\cos(3\phi)$.

**The $L=0$ theorem.** Since $H^0(\mathbb{CP}^3,\mathcal O(0))=\mathbb C$, its projectivization is a point; pure Chern-zero quantization cannot contain a relative-phase clock. The displayed $M_0$ orbit has $z_2(t)=0$ and therefore already lies in the minimal three-state completion

$$M_0^{\rm min}=\omega\operatorname{diag}(0,-1,0),\qquad z_0^{\rm min}(0)=\left(A,\frac{B}{\sqrt2},\frac{B}{\sqrt2}\right),$$

which gives one constant amplitude state plus the two states required for a gauge-invariant clock and hence $X+iY=Re^{i\omega t}$. This settles the ring trajectory but proves that its clock is extra universal structure, not a consequence of $\mathcal O(0)$ flux. A uniform one-bundle construction is impossible at degree $L$ by the one-dimensional $L=0$ space. It is also impossible at degree $L+1$: the $L=0$ target fixes fundamental weight diameter one, so degree-$(L+1)$ weights span at most $L+1$, less than the required extremal diameter $2L$ for $L\geq2$. Degree $L+2$ is the first algebraically possible offset; that option changes the Chern flux and is not selected.

**Physical status.** The construction proves that higher-dimensional $U(1)$ magnetism and projective combinatorics can generate the required locked flow for every $L\geq1$; the generator, primitive clock, and equality of the nonnegative flux and angular integers are fixed at the stated representation premises. What remains is an action-level selection law showing why the bound state occupies a particular $L$, the orientation map $\Phi:\Xi_6\to\mathbb{CP}^3$ that makes $F^{\rm phys}=(\hbar/e)\Phi^*\mathcal F$ an electromagnetic rather than merely Berry curvature, the dimensional rate $\omega$, the full detector embedding, and the transverse profile. Topology cannot fix $\omega$: rescaling the Hamiltonian preserves its Chern class, weights, curve, and lobe count while changing only physical time. If the full $L$ flux is mapped uniformly through an observed orbit disk, flux quantization and $mvr=L\hbar$ give $\omega_B=\omega_{\rm orb}$; that disk identification remains a consistent dynamical target. The $L=0$ trajectory is mathematically minimal, but the physical origin and ordinary-angular-momentum cancellation of its universal clock remain open. 🔶
