...
| **CKM unitarity exact at tree level** | |Vud|²+|Vus|²+|Vub|² = 1.000013 (+0.001%) from IDWT bare couplings. The literature +5.5σ tension is in the nuclear-QED-corrected extracted value, not in IDWT: the nuclear β-decay QED correction δ_R ≈ 0.024 is applied before extraction and is external to IDWT. IDWT satisfies exact CKM unitarity. |
...
**PMNS angles are a loop-level prediction.** At tree level in IDWT, the W coupling amplitude A_{αi} = ⟨χ_{n_W,2}|K_{2d_α}|χ_{n_α,d_α}⟩ × ⟨χ_{n_W,2}|K_{25}|χ_{n_i,5}⟩ factorises into a product of two independent sector overlaps. The resulting |U_{αi}|² matrix has rank 1 — it cannot be a non-trivial unitary matrix. At tree level, IDWT predicts the identity PMNS matrix. The observed large PMNS mixing angles must therefore arise at next-to-leading order from loop corrections to the kernel. The loop corrections involve the IR-divergent neutrino modes (low-mass d=5 states are near-zero modes) and are expected to be O(1) — consistent with the observed large mixing. The loop integral has not been computed.
...
**g₂, α, G_F, v — now derived ✅.** The SU(2)_L coupling is determined by the CP²→CP¹ sector reduction: g₂ = Q_u √g_s = (2/3)√(2g₄₄/π²) = 0.65275 (PDG: 0.65270, +0.008%). This gives v = 2m_W/g₂ = 246.28 GeV (+0.023%), G_F = 1/(√2 v²) = 1.1658×10⁻⁵ GeV⁻² (−0.05%), and λ_H = 0.1294 (+0.3%). The fiber-scale α gives 1/α = 131.8; after hadronic vacuum polarisation (≈3.8 units) between m_W and m_Z, the running gives 1/α(m_Z) ≈ 127.9 (PDG: 127.9). W total width Γ_W = 2044 MeV (−2%), Z total width Γ_Z = 2517 MeV (+0.9%), muon lifetime τ_μ = 2.190×10⁻⁶ s (−0.3%). See Part 3 §0.7. Open: g₁ has −1.95% residual. After 1-loop U(1)_Y running from the fiber scale to m_Z, the residual is −1.88% — not fully closed. The remaining ~0.1% requires 2-loop QED matching between the IDWT fiber scheme and MS-bar. The calculation is defined but not yet performed.
