...
At the Gegenbauer critical point this means two things. No gap: the tau can always find a decay channel (explaining its short lifetime). No dominant channel: coupling weight is distributed and suppressed at every specific channel, so no single mode carries significant probability (explaining why the lifetime is finite).

The geometric back-reaction correction $\delta_\tau = 1/1680$ required for the tau's mass [1] is the mathematical signature of the Gegenbauer critical point: an all-orders resummation is necessary only at the critical boundary, where the naive perturbation series does not converge.

The $d=10$ filter: nothing specifically blocked; everything barely available via a Gegenbauer-critical coupling that is simultaneously universal and marginal.
