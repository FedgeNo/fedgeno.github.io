...
```
This is a heuristic estimate. The rigorous derivation from the sector heat kernel is in §2.9, which gives A_rel = exp(−c_d λ̂_d) with c_d = d/(d+1)² and λ̂_d = √λ_d (ranging from 0.4 to 7.1 across sectors; the previous claim λ̂_d ≈ 1 was an artifact of the discarded saturating-potential ansatz).
...
**Stage-2 stability: eigenvalue selection by coupling closure. 🔵**
...
The strange quark ($n=4$, $d=3$, odd $l$) is similarly protected from intra-sector coupling to the seeds, but is *stabilised by cross-sector coupling* $V_{34}$: the up quark ($n=3$, $d=4$) sits at level $N=2$ in the $d=4$ oscillator, which has an $l=0$ component, making the cross-sector coupling ($n=1,d=3$) $\to$ ($n=3,d=4$) allowed. The generation tower chain then reaches ($n=4,d=3$) from ($n=3,d=4$) via the hockey-stick identity $n_{\rm strange} = n_s = S(n_u,3)/S(n_u,3)$... establishing that the two seeds mutually stabilise each other through the cross-sector coupling.
