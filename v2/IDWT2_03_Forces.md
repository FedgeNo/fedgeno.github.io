...
The scale check falls out at once: a connecting ripple at the colour scale has wavelength $\sim1/\lambda_c$ and energy $\sim\lambda_c$ per fringe, so its tension is $\sigma \sim \lambda_c \div (1/\lambda_c) = \lambda_c^2$ — exactly the relation verified against $\sqrt\sigma$, the Regge slope, and $T_c$ above. The mechanism predicts the *square* that the scale-match found.

**The repulsive core, and the full shape of the potential.** Attraction is half the story. When two quark patterns *overlap*, the density-against-density interference energy is positive — the kernel is a sum of squares (Part 1 §1.2), so stacking two cores in the same territory costs energy — and that cost fades with separation, on the contact scale, because it exists only where the cores actually overlap. The net interquark force is therefore a constant string pull $\sigma$ minus a short-range repulsion that dies off:

- **Hadrons have a size** because the fading repulsion balances the constant pull at a definite separation — the contact scale, sub-femtometre, where the proton in fact sits. Without the core, the string would crush the quarks to a point.
- **Quarks act nearly free inside** because near that balance the two terms nearly cancel — a shallow net potential, the framework's native counterpart of asymptotic freedom (here from geometric dilution and core–string cancellation, not from a running coupling).
- **The attraction "strengthens" with distance** without any force strengthening: the constant pull is progressively *unmasked* as the overlap repulsion fades, until the bare tension is all that remains — and at sufficient stretch, creating a new quark pair is cheaper than another fringe, which is why pulling on quarks yields hadrons, never free colour.

Cross-framework note: decaying short-range piece plus linear piece is precisely the empirical heavy-quark potential (the Cornell form) that lattice QCD measures; the framework arrives at that shape natively. Status, plainly: the admissibility rule and the $\sigma = \lambda_c^2$ scale are committed results; the fringe mechanism and core–string balance are a *candidate* account — natively built, shape- and scale-correct, awaiting the kernel-level computation of the fringe energy density and the coefficient.
...
*Second edition, 2026. Full derivations and machine verification: first-edition Parts 3, 8, 10 and `files/idwt.py` at https://fedgeno.github.io/.*
