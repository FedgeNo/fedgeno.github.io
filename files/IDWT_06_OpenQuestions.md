...
**Invisible population — enumeration complete.** The Stage-2-pass / Stage-1-fail population (modes with n ∈ Σ, d ∈ {5,6,10}, not a physical SM particle, Ω_log > ln 2) has been fully enumerated. There are exactly 36 such modes across three sectors, named:
- **Velinos** (v̄₁–v̄₁₁, d=5): 11 modes, masses 15.6 peV – 53.1 μeV. Named from Latin *velum* (veil).
- **Umbrons** (Ū₁–Ū₁₂, d=6): 12 modes, masses 771 μeV – 32.8 GeV. Named from Latin *umbra* (shadow).
- **Liminons** (Lm₁–Lm₁₃, d=10): 13 modes, masses 1.82 keV – 718 TeV. Named from Latin *limen* (threshold), referencing the Gegenbauer critical point T5.

All masses are m = S(n,d) × m_scale_d with the same mass scales as the corresponding physical sector particles. These modes are geometrically stable (Stage 2) and gravitationally present (P5 is sector-agnostic) but have negligible d=3 amplitude. Full mass tables in Part 7 §2.6.
