...
| Scale hierarchy (OQ17) | m_scale_3 = m_e × √(g₃₃/g₆₆) = 4.702 MeV from the kernel vacuum fixed-point equation (l=0 scalar part of the cross-sector coupling). Requires only m_e and coupling constants derived from seeds and anomaly cancellation. All other sector scales follow from m_scale_3 or m_W. |
| ρ meson from comb filter (OQ35) | Im[Γ₃₄₆] maximum at 775.794 MeV (+0.069% vs PDG 775.260 MeV). Inputs: g₃₃=8√7, g₄₄=12/√7, g₆₆=1/4, τ_d=1/(2√(k₀+d)) at k₀=16. Consistency check — the same coupling constants that fix m_scale_3 also determine this peak, so agreement is a self-consistency verification. The +0.069% is a consistency-level residual: WKB correction to τ_d for d=3 goes in the wrong direction, and is zero for d=10 by the Gegenbauer criticality theorem. |
...
The d=5 coupling is algebraically closed: g₅₅ = 96/g₂₂ = 0.1329. The naive coupling value gives m_scale_5 ≈ 0.37 MeV, while observed neutrino masses are ~meV — a suppression of ~5×10¹¹. The seesaw mechanism is geometrically forbidden (Majorana spinors do not exist in d=5, d mod 8 = 5), so this suppression must arise entirely from the d=5 sector vacuum dynamics: non-perturbative suppression at λ̂₅ ≫ 1, or a condensate from the (5,5)→10 Vandermonde vertex. Neutrino mass RATIOS are fully predicted (S(10,5):S(15,5):S(22,5)). The absolute mass scale mechanism is open.
...
**4D gravity from M_∞ action (formal) 🔶**
The qualitative argument that IDWT's macroscopic hidden dimensions are consistent with gravitational experiments rests on the absence of gravitons — gravity is purely geometric curvature of the projected 4D slice, so there is nothing to propagate into Ξ. This argument is sound (Part 4 §1b), but the formal derivation is open: showing explicitly from the M_∞ action that varying with respect to the visible metric g_μν produces 4D Einstein equations without independent gravitational propagating degrees of freedom from the hidden sector. The specific requirement: prove that hidden metric fluctuations in Ξ do not contribute extra propagating modes to the projected gravitational field — only to the effective stress-energy through the mass spectrum and vacuum energy already accounted for in T_μν^obs.
