...
**Selection Principle T0.5.** A mode $(n,d)$ is a physical particle if and only if the pair $(n,d)$ is an element of $\Sigma_{\rm pairs}$ — the co-fixed-point set produced by the generation tower (the sector comb filtration generated by $n_s$). **Status: 🔶** semi-structural: the instability of non-co-fixed-point modes (l-parity disconnection for odd levels, infinite-dimensional dephasing on timescale $1/m_{\mathrm{scale},d}$ for even levels) is established at the kernel level (Part 7 §1.2) but the decoherence is not yet derived from the IDWT equation of motion. The construction is correct in its outcomes but not a proved theorem at current rigor.
