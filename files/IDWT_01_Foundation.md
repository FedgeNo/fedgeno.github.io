...
S³ → S⁷  → S⁴    quaternionic     →  d=4 (base S⁴≅HP¹), d=5 (total S⁵ over CP²)
S⁷ → S¹⁵ → S⁸   octonionic       →  d=10 (total space, via 𝕆P¹)
```

d=6 arises from the complex Hopf fibration applied to S⁵: S¹ → S⁵ → CP², giving a second d=6 mode space CP³ as the twistor space over S⁴. d=7 is absent because S⁷ is consumed as the quaternionic fiber, not producing an independent sector. d=8 would correspond to GUT coset CP⁴=SU(5)/U(4), absent because SU(5) symmetry breaks at the GUT scale.
...
‡ After applying the Generation Tower Correction (Part 2 §11) with ε = 1/(280√7) and k values {charm:3, top:10}, these become 0.000% and −0.039% respectively. The d=4 within-sector residuals are a GTC issue, not a scale issue.

**Co-fixed-point uniqueness (§23.9d) ✅**
