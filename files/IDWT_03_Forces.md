...
|---|---|---|---|---|---|---|---|
| 2 | $\mathbb{CP}^1$ | $SU(2)$ | 2 | $722.5$ | 27.47 MeV | $\gamma,\;W^\pm,\;Z^0,\;H$ | Orientation filter: two helicity states = complete internal geometry; coupling $\propto \varepsilon_\mu j^\mu$, zero for $j^\mu \perp \varepsilon_\mu$ |
...
This is a state-count (IDOS) ratio — the framework's native currency, on the same footing as the mass law, which is itself the integrated density of states (Part 8 §5, Theorem S1). It is not the $T2$ kernel overlap $\langle\chi|K_{dd}|\chi\rangle$, which scales as $n^1$ and is not the object that sets mixing (Part 6).

**Counting-theorem skeleton (🔶, `files/idwt.py` STEP 131).** The ratio law follows from three verified pieces. *(i) Equidistribution:* a pointlike excitation carries equal norm per state across its cumulative tower of $S(n,d)$ states — the addition-theorem mechanism proved for the $g_A$ denominator (Part 8 §10). *(ii) Fixed kernel channel:* the kernel's angular factor satisfies $t^2 \in \mathrm{span}\{C_0^{\alpha}, C_2^{\alpha}\}$, $\alpha=(d-2)/2$, exactly, in every sector $d \in \{3,4,5,6,10\}$ — an $n$-independent channel of multiplicity $m_0$, so a mode's channel amplitude is $\sqrt{m_0/S(n,d)}$. *(iii) Conditional Born ratio:* the transition weight is the destination channel intensity relative to the source, in which $m_0$ and the global amplitude cancel: $|V|^2 = S_{\rm lo}/S_{\rm hi}$. The normalization is data-selected between the two Born-like readings: the normalized-flavor alternative ($\tan^2\theta = S_{\rm lo}/S_{\rm hi}$) is excluded by the Cabibbo datum at $-6\sigma$, while the conditional ratio is confirmed at $+0.3\sigma$. The conditional normalization is derived at the vertex level (`files/idwt.py` STEP 134): the mixing element and its reference come from the same quartic kernel vertex, so $|A_{ij}|^2/|A_{ii}|^2 = I_j/I_i$ with the channel multiplicity and global amplitude cancelling exactly, and rate conservation fixes the pair $(|V_{\rm mix}|^2, |V_{\rm diag}|^2) = (r, 1-r)$ — one angle, $\sin^2\theta = r$, the data-selected reading. A consequence: exact first-row unitarity is an IDWT prediction (Part 5 C7); the current PDG $|V_{ud}|$ sits $+2.5\sigma$ from $\sqrt{1-r(1+1/240)^2}$, the known first-row-anomaly territory — an open residual. The $\mathbb{CP}^k$ write-out of L1 is now explicit: the level-1 addition-theorem power sum $\sum_{ij}(Z_i\bar Z_j - \delta_{ij}/(n+1))^2 = n/(n+1)$ is position-constant on every $\mathbb{CP}^n$ sector manifold ($d = 2,4,6,10$; verified exactly, `files/idwt.py` STEP 131), so the equidistribution mechanism proved on $S^3$ (Part 8 §10) holds on all sector manifolds. The cross-sector law is the same ratio carried across sectors — the $|V_{ub}|$ apex (STEP 136) is a $d=4$ source count over a $d=3$ destination count — and $g_{22} = p^2 q/2$ is its multiplicative form, a $d=3$ count squared times a $d=4$ count (STEP 2d). That transition weights are channel quantities at all — rather than properties of any other decomposition of the modes — is the Channel-Projection Lemma (`files/idwt.py` STEP 139): observables are functions of mode frequencies and kernel matrix elements only, so the components entering any transition are the kernel-channel projections by construction. Remaining open (🔶): the L1 pointlike-equidistribution premise itself (the size-program thesis, Part 4 §3.9a).
...
🔶 All boson mode indices follow from the $g$-rule sector coupling $g(a,b) = a + b - 1$ applied to occupied mode indices and sector dimensions:
...
**Recoil form and boundary pinning. 🔶** The same structure reads as four-wave index conservation: every pair composition in the spectrum satisfies $n_a + n_b = n_c + n_{\rm recoil}$ exactly, with recoil either the photon ($n{=}0$: the one-insertion fermion edges) or the condensate ground ($n{=}1$: the $g$-rule joins — the shared vacuum unit above is the ground-recoil quantum), and the choice is the even-insertion rule itself: recoil parity is fixed by the landing sector (`files/idwt.py` STEP 142). In level variables $N = n-1$ the $g$-rule is pure addition, $N_H = N_{\nu_2} + N_Z$. Independently of the route, each boson index is pinned by the electroweak subscription bound: given the other heavy indices, $n_W = 76$, $n_Z = 81$, and $n_H = 95$ are the unique integers at the subscription boundary — 75 undersupplies the capacity side while 82 and 96 oversubscribe the channel quantum (Part 9 T7 constraint form, `files/idwt.py` STEP 141).
...
This is a theorem of $n_s=4$ and the $g$-rule structure, determined entirely by the combinatorial fixed point.
...
**Generation number:** $N_{\rm gen} = N_c = 3$ follows from $n_s = \chi(\mathbb{CP}^3) = 4$ and the triviality of the identity class: the $d=6$ lepton sector's four $\mathbb{CP}^3$ cohomology classes are $\{1, \omega, \omega^2, \omega^3\}$ (powers of the Kähler class). The unit class $1$ is the constant/vacuum mode — the same trivial role the unit/empty-product class plays in the Kähler-ring deposit structure (Part 2 §15) — so it is not a physical generation, leaving the three non-trivial Kähler powers $\{\omega, \omega^2, \omega^3\}$: $N_{\rm gen} = n_s - 1 = 3$. Since $n_s = N_c + 1$ (T4/T15a chain: $N_c = \chi(\mathbb{CP}^2) = 3$, $n_s = \chi(\mathbb{CP}^3) = 4$), this gives $N_{\rm gen} = N_c = 3$ by construction. **Status: ✅.**

*Division-algebra identity (⭐).* The charged-lepton sector $d=6$ sits at $d-2 = 4 = \dim\mathbb{H}$ in the sector ladder, and $\chi(\mathbb{CP}^3) = 4 = \dim\mathbb{H}$, so the generation count also reads $N_{\rm gen} = \chi(\mathbb{CP}^3) - 1 = \dim\mathbb{H} - 1 = \dim\,\mathrm{Im}\,\mathbb{H} = 3$. This is an algebraic restatement of the same count, not a second derivation — the mechanism is the identity-class removal above, of which dropping the real unit of $\mathbb{H}$ is the faithful division-algebra image ($\mathbb{H} = \mathbb{R}\cdot 1 \oplus \mathrm{Im}\,\mathbb{H}$; the real unit $1 \leftrightarrow$ the trivial cohomology class). It is noted because the sector set realizes the normed division algebras at $d-2 \in \{1,2,4,8\}$, and under that reading three generations is the imaginary dimension of the quaternions.
