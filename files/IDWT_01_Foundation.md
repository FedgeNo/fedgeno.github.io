...
A mode-sector pair $(n,d)$ is a stable resonance if and only if it is an element of $\Sigma_{\rm pairs}$, the co-fixed-point set: the unique finite set of pairs such that applying every generation-tower operation to $\Sigma_{\rm pairs}$ returns only elements already in $\Sigma_{\rm pairs}$ (verified exhaustively, §5c). The generation tower is a finite acyclic DAG with seeds $\{(1,3),(3,4)\}$ (composite $(4,3)$ at depth 1) and trivial automorphism group — the labeling of all 15 particles is uniquely determined by the DAG structure.
...
```
n_top = χ(CP²) × χ(CP³) × χ(CP⁵) = N_c × n_s × N_f = 3 × 4 × 6 = 72  [⭐ identity for the value; seed input — accepted on the same footing as n_d=1 and n_u=3; see §3a and Part 9 T15b]
```
...
| R2 | d=4 | up, charm, top | n_u = χ(CP²) = N_c = 3 (T15, ✅); n_top = χ(CP²)×χ(CP³)×χ(CP⁵) = 72 (⭐ value identity — top Chern number; seed input, §3a) |
...
- **PMNS angles:** θ₁₂, θ₂₃, θ₁₃ determined by g₅₅ = 96/g₂₂ and the four mode indices (n_e, n_μ, n_τ, n_ν); all three angles determined by sector geometry (Part 9 T6).
